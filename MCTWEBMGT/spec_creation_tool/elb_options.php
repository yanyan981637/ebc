...
?>
<tr><td ><input name="optons_name" type="hidden" value="<?=$str_s;?>">
<input name="opt_all[]" type="checkbox" value="<?=$SPECOptionID;?>" <?php if(strpos($data_option_ss,$SPECOptionID.",")!='' || strpos($data_option_ss,$SPECOptionID.",")===0){ echo "checked"; } ?> /> <?=$SPECOptionValue;?></td></tr>
<?php
 }
 mysqli_close($link_db);
?>
<tr><td><p style="padding:5px 20px; "><input type="hidden" name="p_id" value="<?=$p_SKU?>"><input type="hidden" name="sp_id" value="<?=$SPCT_ID?>"><input type="submit" value="Done"  /></p></td></tr>
</tbody>
</table>
</form>
