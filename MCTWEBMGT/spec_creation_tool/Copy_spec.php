...
//$select=mysqli_select_db($dataBase, $link_db);
    ...
    echo "<script>self.location='Copy_spec.php?p_id=".$u1."&p_PMA".$PA_1."=".$PA_1."&p_seVal".$PA_1."=".$s1."&m_sort=".$sstr1."&get_cookies=Yes#product_matrix';</script>";
    exit();
}else{
    echo "<script>self.location='Copy_spec.php?p_id=".$u1."&p_PMA".$PA_1."=".$PA_1."&p_seVal".$PA_1."=".$s1."&m_sort=".$sstr1."#product_matrix';</script>";
    exit();
}
...
if(trim($_REQUEST['methods'])=='Copy_spec'){
...
$MCount=$Max_CSKUID[0]+1;
    ...
    unset($_SESSION["SEL_PMatrix01"]);
        ...
        for($S=3;$S<17;$S++){
          ...
          $str_item="SEL_PMT0".$S;
          $str_item_M="PMS_".$S;
          if($_POST[$str_item]<>'' && $_POST[$str_item]<>'Add'){
          $str="'".$_POST[$str_item]."',";
          }else{
...
//*********** product_models 寫入sku 跟vw_modelname連動 ***************
if(isset($_POST['MN_03'])!=''){
$MN_03=trim($_POST['MN_03']);
}else{
$MN_03="";
}

$select_pmodels =  "SELECT * FROM `product_models` WHERE `ModelCode`='".$SEL_PMODEL."' and `SKU` IS NULL ";
$cmd_pmodels=mysqli_query($link_db,$select_pmodels);
$result_pmodels=mysqli_num_rows($cmd_pmodels);
$result_pm=mysqli_fetch_row($cmd_pmodels);
if($result_pmodels>0){
  $update = "update product_models set SKU='".$SKU_value."', upd_d='".$now."' where ModelID='".$result_pm[0]."' and ModelCode='".$result_pm[2]."' ";
  $u_cmd=mysqli_query($link_db,$update);
}else{
  $insert = "INSERT INTO `product_models`(ModelCode, SKU, ProductCateID, crea_d) VALUES ('".$SEL_PMODEL."', '".$SKU_value."', '".$MN_03."', '".$now."')";
  $i_cmd=mysqli_query($link_db,$insert);
}
//**************** product_models 寫入sku end***************
...
$str_P_emb="select `EMBEDDEDID` FROM `p_b_main_embedded` order by `EMBEDDEDID` desc limit 1";
$check_P_emb=mysqli_query($link_db,$str_P_emb);
$Max_P_embMaxtrixID=mysqli_fetch_row($check_P_emb);
$embMXCount=$Max_P_embMaxtrixID[0]+1;
...
$str_P_5G="select `5GID` FROM `p_b_main_5G` order by `5GID` desc limit 1";
$check_P_5G=mysqli_query($link_db,$str_P_5G);
$Max_P_5GID=mysqli_fetch_row($check_P_5G);
$MXCount_5G=$Max_P_5GID[0]+1;

$str_P_5G="select `IntelDSGID` FROM `p_b_main_inteldsg` order by `IntelDSGID` desc limit 1";
$check_P_5G=mysqli_query($link_db,$str_P_5G);
$Max_P_5GID=mysqli_fetch_row($check_P_5G);
$MXCount_IntelDSGID=$Max_P_IntelDSGID[0]+1;
...
//**************Edit Save 執行**************
  ...
  if(intval($_POST['PT1'])==101 || intval($_POST['PT1'])==102){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `NetWorking`, `SAS`, `FormFactor`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PNWork','$SEL_SAS','$SEL_FFactor','$UPC_value','',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,1,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_s_main_systemboards` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
       $str_prods="";
   }else{
       $str_prods="INSERT INTO `p_s_main_systemboards`(`SYSTEMBOARDID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$MBMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }

  }else if(intval($_POST['PT1'])==103){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `PCI-E_slot`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PCIE_slot','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,2,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_serverbarebones` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
       $str_prods="";
   }else{
       $str_prods="INSERT INTO `p_b_main_serverbarebones`(`SERVERID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$BBMXCount.",'','".$SEL_PMODEL."','en-US','0')";    
   }
  }else if(intval($_POST['PT1'])==104){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,2,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
  ...
  }else if(intval($_POST['PT1'])==105 || intval($_POST['PT1'])==106){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Host_IF`, `Conn_Type`, `Conn_Qty`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_Host_IF','$SEL_C_Type','$SEL_C_Qty','$UPC_value','',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `MODELDESCRIPT`) VALUES ($MCount,0,3,'$SKU_value','$SEL_PMODEL',$SEL_SKU_S,'EN,','$now','1782',$PT1,'$note01')";
    if(isset($_POST['PT1'])==105){
      $p_select = "SELECT * FROM `p_s_main_nic` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
      $p_cmd=mysqli_query($link_db,$p_select);
      $nums=mysqli_num_rows($p_cmd);
      if($nums > 0){
          $str_prods="";
      }else{
          $str_prods="INSERT INTO `p_s_main_nic`(`NICID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$NICMXCount.",'TYAN','".$SEL_PMODEL."','en-US','0')";
      }
    }else if(intval($_POST['PT1'])==106){
      $p_select = "SELECT * FROM `p_s_main_hba` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
      $p_cmd=mysqli_query($link_db,$p_select);
      $nums=mysqli_num_rows($p_cmd);
      if($nums > 0){
          $str_prods="";
      }else{
          $str_prods="INSERT INTO `p_s_main_hba`(`HBAID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$HBAMXCount.",'TYAN','".$SEL_PMODEL."','en-US','0')";
      }
	  }
  }else if(intval($_POST['PT1'])==107){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,46,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_panelpc` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_panelpc`(`PANELPCID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$panelMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==108){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,47,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_embedded` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_embedded`(`EMBEDDEDID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$embMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==109){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,48,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_industriamb` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_industriamb`(`INDUSTRIAMBID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$industriaMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==110){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,49,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_ocpserver` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_ocpserver`(`OCPID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$OCPMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==111){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,50,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_ocpmezz` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_ocpmezz`(`OCPMezzID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$OCPMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==112){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,51,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_jbodjbof` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_jbodjbof`(`JBODFID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$JBODFMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==113){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,52,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_ocprack` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_ocprack`(`OCPRACKID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$OCPRACKMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==114){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,53,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_pos` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_pos`(`POSID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$posMXCount.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==115){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,54,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_5G` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_5G`(`5GID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$MXCount_IntelDSGID.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }else if(intval($_POST['PT1'])==116){
   $str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `Riser`, `HDD_Bay`, `PSU`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$UPC_value','$SEL_FAN',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$note01')";
   $str_con_skus="INSERT INTO `contents_product_skus`(`Product_SContents_Auto_ID`, `CategoryModuID`, `ProductTypeID`, `SKU`, `MODELCODE`, `STATUS`, `slang`, `crea_d`, `crea_u`, `ProductTypeID_SKU`, `lang_status`, `MODELDESCRIPT`) VALUES (".$MCount.",0,55,'".$SKU_value."','".$SEL_PMODEL."',0,'EN,','".$now."','1782',".$PT1.",1,'".$note01."')";
   
   $p_select = "SELECT * FROM `p_b_main_inteldsg` WHERE `MODELCODE` = '".$SEL_PMODEL."'";
   $p_cmd=mysqli_query($link_db,$p_select);
   $nums=mysqli_num_rows($p_cmd);
   if($nums > 0){
      $str_prods="";
   }else{
      $str_prods="INSERT INTO `p_b_main_inteldsg`(`IntelDSGID`, `MODELNAME`, `MODELCODE`,`LANG`,`STATUS`) values (".$MXCount_IntelDSGID.",'','".$SEL_PMODEL."','en-US','0')";
   }
  }
...
/*
$str_skuu="INSERT INTO `product_skus`(`Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `NetWorking`, `SAS`, `FormFactor`, `Riser`, `HDD_Bay`, `PSU`, `Host_IF`, `Conn_Type`, `Conn_Qty`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `slang`, `SKU_CategorySort`, `SKU_Type`, `PCI-E_slot`, `Chip`) VALUES ($MCount,$PT1,'$SKU_value','$SEL_PMODEL','$SEL_PNWork','$SEL_SAS','$SEL_FFactor','$SEL_PRiser','$SEL_HDD_Bay','$SEL_PSU','$SEL_Host_IF','$SEL_C_Type','$SEL_C_Qty','$UPC_value','',$specEOL,$SEL_SKU_S,'$guid','$now','1782','$now','$str_lang','$SPECC_Sort_tr','$SPECTP_str','$SEL_PCIE_slot','$SEL_Chip')";
$cmd_skuu=mysqli_query($link_db,$str_skuu);
*/
...
foreach($_POST['aspectype'] as $check_type) {
  ...
  foreach($_POST['aspecoption'] as $check_option) {
    ...
    if(preg_match("/".$check_option."/i",$str_check)!=''){
      ...
      $str_result=str_replace($str_check, '', $check_option);
...
if($_REQUEST['kinds']=='Categroies_set'){
$link_db=mysqli_connect($db_host,$db_user,$db_pwd,$dataBase);
mysqli_query($link_db,'SET NAMES utf8');
mysqli_query($link_db, 'SET CHARACTER_SET_CLIENT=utf8');
mysqli_query($link_db, 'SET CHARACTER_SET_RESULTS=utf8');
...
echo "<script>alert('Set SPECCategroies be ok !');self.location='default.php'</script>";
exit();
...
<title>SPEC Creation Tool - Copy a SPEC</title>
...
if(document.form1.SKU_value.value == "" || document.form1.SKU_value.value == SKU_O01) {
$("#Model_error").html('');
...
<div id="Search" >
<h2><a href="default.php" >Product SPEC</a>&nbsp;&gt;&nbsp; Copy a SPEC </h2> 
</div>
<?php
if(isset($_REQUEST['p_id'])!=''){
$p_SKU=trim($_REQUEST['p_id']);
}else{
$p_SKU="";
} 
  ...
  mysqli_query($link_db, 'SET CHARACTER_SET_RESULTS=utf8');
  //$select=mysqli_select_db($dataBase, $link_db);  
  $str_sku_m="select `Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE`, `NetWorking`, `SAS`, `FormFactor`, `Riser`, `HDD_Bay`, `PSU`, `Host_IF`, `Conn_Type`, `Conn_Qty`, `UPCcode`, `FAN`, `IS_EOL`, `Web_Disable`, `GUID`, `crea_d`, `crea_u`, `upd_d`, `upd_u`, `slang`, `SKU_CategorySort`, `SKU_Type`, `MODELDESCRIPT`, `PCI-E_slot`, `Chip` from product_skus where Product_SKU_Auto_ID=".$p_SKU;
  $cmd_sku_m=mysqli_query($link_db,$str_sku_m);
  $record_sku_m=mysqli_fetch_row($cmd_sku_m);
  
  if(empty($record_sku_m)):
  else:
    $SM0="";$SM1="";$SM2="";$SM3="";$SM4="";$SM5="";$SM6="";$SM7="";$SM8="";$SM9="";$SM10="";$SM11="";$SM12="";$SM13="";$SM14="";$SM15="";
    $SM0=$record_sku_m[1];
    $SM1=$record_sku_m[2];
    $SM2=$record_sku_m[3];
...
?>
<form id="form1" name="form1" method="post" action="?methods=Copy_spec" onsubmit="return SPEC_Check();">
...
?>
<option value="Copy_spec.php?PCate_id=<?=$ProductCateID;?>&PType_id=<?=$ProductTypeID?>" <?php if($EPType_id==$ProductTypeID || $SM0==$ProductTypeID){ echo "selected"; }?> ><?=$ProductCateName?></option>
<?php
mysqli_close($link_db);
?>
</select><div id="PTYPE_error"></div>
<?php
$link_db=mysqli_connect($db_host,$db_user,$db_pwd,$dataBase);
mysqli_query($link_db,'SET NAMES utf8');
mysqli_query($link_db, 'SET CHARACTER_SET_CLIENT=utf8');
...
<input type="hidden" name="PT1" value="<?=$EPType_id;?>">&nbsp;
  ...
  <tr style="background:#fcd2e4">
    <th>Model:</th>
    <td>
	<div style="color:#c00; font-weight:bold;display:none">(** Be sure to add/edit after finishing "SPEC Settings".) </div>
    ...
    //$select=mysqli_select_db($dataBase, $link_db);
    if(isset($_REQUEST['PCate_id'])<>''){
    $str_model="select ModelCode from product_models where ProductCateID=".intval($_REQUEST['PCate_id'])." order by ModelCode";
    }else{
	$str_model="select ModelCode from product_models where ProductCateID=".$ProductCateID." order by ModelCode";
    }
    $Model_result=mysqli_query($link_db,$str_model);
	while(list($ModelCode)=mysqli_fetch_row($Model_result)){
    ?>
    <option value="<?=$ModelCode;?>" <?php if($SM2==$ModelCode) { echo "selected"; } ?>><?=$ModelCode;?></option>
    <?php
	}
    mysqli_close($link_db);
    ?>
    </select>&nbsp;&nbsp;&nbsp;&nbsp; <div id="model_add01" style="display:none">Model Name: <input type="text" name="MN_01" size="20" value="TYAN"> Model Code: <input type="text" name="MN_02" size="20"> <input type="hidden" name="MN_03" value="<?=$EPCate_id?>"><input id="MNBtn" type="button" value="Done" /></div><div id="Model_MGT"></div><div id="Model_error"></div>
  ...
  <tr style="background:#fcd2e4">
    ...
    <td>
	<div style="color:#c00; font-weight:bold;display:none">(** Be sure to add/edit after finishing "SPEC Settings".) </div>
	<p><input id="SKU_value" name="SKU_value" type="text" size="30" value="" />&nbsp;&nbsp;<font color="red"><span id="SKU_Orign"><?=$SM1;?></span></font>&nbsp;&nbsp;<div id="divAccount"></div><div id="SKU_error"></div></p>
...
<thead>
  <tr>
    <th >Categories <a href="Categories_ESortable.php?SKU_id=<?=$p_SKU?>&PType_id=<?=$EPType_id;?>" class="fancybox fancybox.iframe"> <font size="2">(Order)</font></a></th><th >Types / Options</th> 
  </tr>
</thead>

<?php
$data_SPEC="";$data_type_s="";$data_SPEC_cs="";$data_option_s="";$data_option_s="";$data_optionc_s="";$data_SPEC1="";$data_spec_str="";

$SPECType_numAll="";$SPECType_num="";$ParentSpec_va_all_Sub="";$ParentSpec_va_all_Thr="";
...
$SPECCat_result_cs=mysqli_query($link_db,$str_SPECCat_cs);
...
$data_option_s=str_replace(',,', ',', $data_option_s);
...
<tbody>
	<tr >
	<td><!--<input name="checkall" type="checkbox" value="<?=$data_spec[0];?>" <?php if($data_spec[2]=='1') echo "checked"; ?> onclick="check_id(<?=$data_spec[0];?>,'<?=$data_spec[1];?>',<?=$i;?>);" />--> <a class="fancybox fancybox.iframe" href="elb_categories.php?p_id=<?=$p_SKU;?>&PType_id=<?=$EPType_id;?>"><?=$data_spec[1];?></a></td>
  ...
  if(isset($_COOKIE["type_cookie".$data_spec[0].""])!=''){	
	...
	$str_Types="select * from spectypes where SPECCategoryID=".$data_spec[0]." and INSTR('".$SPECTypeID_va_all.$ParentSpec_va_all."',SPECTypeID)>0 order by SPECTypeSort";
  ...
  $str_Types="select * from spectypes where SPECCategoryID=".$data_spec[0]." and INSTR('".$data_p[1]."',SPECTypeID)>0 order by SPECTypeSort";
  }  
  ...
  ?>
           ...
           $ii1=$ii1+1;
			 ...
			 if(preg_match("/".$Tdata[0]."/i",$SM14)!=''){
              ...
              $strr_option="<div id='divObj".$i1."' display:'inline-block'>";
              $j=0;
			  $str_Optionss="select SPECOptionID,SPECOptionValue from specoptions where SPECTypeID=".$Tdata[0]." and INSTR('".$data_option_s."',SPECOptionID)>0";
              $Optionsresults=mysqli_query($link_db,$str_Optionss);
              if($Optionsresults==true){  
              while(list($SPECOptionID,$SPECOptionValue)=mysqli_fetch_row($Optionsresults)){
              $j=$j+1;
  ...
  ?>
        ...
        <?php
        $j=0;
        $str_Options="select SPECOptionID,SPECOptionValue from specoptions where SPECTypeID=".$SPECTypeID." and INSTR('".$data_option_s."',SPECOptionID)>0";
...
<OPTION value="0" <?php if(ord($SM9)==false){ echo "selected"; }?>>Enabled</OPTION>
<OPTION value="1" <?php if(ord($SM9)==true){ echo "selected"; }?>>Disabled</OPTION>
</select>
<hr class="style-four" /> <p class="clear"></p>
<?php
$PMX1="";$PMX2="";$PMX3="";$PMX4="";$PMX5="";$PMX6="";$PMX7="";$PMX8="";$PMX9="";$PMX10="";
$PMX11="";$PMX12="";$PMX13="";$PMX14="";$PMX15="";$PMX16="";$PMX17="";$PMX18="";$PMX19="";$PMX20="";
if($EPType_id<>""){
...
<h4>Form Factor:</h4>
			...
			if(isset($_SESSION['p_seVal01'])!=""){
               ...
               if($_SESSION['p_seVal01']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX4=="NO"){ echo "selected='selected'"; }
            ...
            <?php 
              if(isset($_SESSION['p_seVal01'])!=""){
               ...
               if($_SESSION['p_seVal01']==$MValue_VName){ echo "selected"; }
               
              }else{
              
               if($PMX4==$MValue_id){ echo "selected"; }
...
<h4>CPU / <?=$Cvalues;?>:</h4>
			...
			if(isset($_SESSION['p_seVal02'])!=""){              
               if($_SESSION['p_seVal02']=="NO"){ echo "selected"; }               
              }else{              
               if($PMX5=="NO"){ echo "selected='selected'"; }               
              }
			?>>NO</option>
            <?php
            $link_db=mysqli_connect($db_host,$db_user,$db_pwd,$dataBase);
            ...
            <?php 
              if(isset($_SESSION['p_seVal02'])!=""){
               ...
               if($_SESSION['p_seVal02']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX5==$MValue_id){ echo "selected"; } 
...
<h4>Chipset:</h4>
			...
			if(isset($_SESSION['p_seVal03'])!=""){
               ...
               if($_SESSION['p_seVal03']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX6=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal03'])!=""){
               ...
               if($_SESSION['p_seVal03']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX6==$MValue_id){ echo "selected"; } 
...
<h4>Exp. Slots -> PCIx:</h4>            
			...
			if(isset($_SESSION['p_seVal04'])!=""){
               ...
               if($_SESSION['p_seVal04']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX7=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal04'])!=""){
               ...
               if($_SESSION['p_seVal04']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX7==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show06" style="display:none"><input type="text" id="PMS_06" name="PMS_06" size="20" value=""><br />Tooltips :<input id="PMS_06U" name="PMS_06U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(6,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Exp. Slots -> PCI:</h4> 
			...
			if(isset($_SESSION['p_seVal05'])!=""){
               ...
               if($_SESSION['p_seVal05']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX8=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal05'])!=""){
               ...
               if($_SESSION['p_seVal05']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX8==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show07" style="display:none"><input type="text" id="PMS_07" name="PMS_07" size="20" value=""><br />Tooltips :<input id="PMS_07U" name="PMS_07U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(7,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Exp. Slots -> PCIe:</h4> 
			...
			if(isset($_SESSION['p_seVal06'])!=""){
               ...
               if($_SESSION['p_seVal06']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX9=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal06'])!=""){
               ...
               if($_SESSION['p_seVal06']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX9==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp;<!--<font color=red><?=$PMX9;?></font>--> <div id="PMT_Show08" style="display:none"><input type="text" id="PMS_08" name="PMS_08" size="20" value=""><br />Tooltips :<input id="PMS_08U" name="PMS_08U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(8,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Memory -> Max.:</h4> 
           <select id="SEL_PMT009" name="SEL_PMT009">
			...
			if(isset($_SESSION['p_seVal07'])!=""){
               ...
               if($_SESSION['p_seVal07']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX10=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal07'])!=""){
               ...
               if($_SESSION['p_seVal07']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX10==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show09" style="display:none"><input type="text" id="PMS_09" name="PMS_09" size="20" value=""><br />Tooltips :<input id="PMS_09U" name="PMS_09U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(9,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Memory -> Type:</h4> 
			...
			if(isset($_SESSION['p_seVal08'])!=""){
               ...
               if($_SESSION['p_seVal08']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX11=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal08'])!=""){
               ...
               if($_SESSION['p_seVal08']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX11==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show10" style="display:none"><input type="text" id="PMS_10" name="PMS_10" size="20" value=""><br />Tooltips :<input id="PMS_10U" name="PMS_10U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(10,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Integrated Features -> Audio (A):</h4> 
			...
			if(isset($_SESSION['p_seVal09'])!=""){
               ...
               if($_SESSION['p_seVal09']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX12=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal09'])!=""){
               ...
               if($_SESSION['p_seVal09']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX12==$MValue_id){ echo "selected"; } 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show11" style="display:none"><input type="text" id="PMS_11" name="PMS_11" size="20" value=""><br />Tooltips :<input id="PMS_11U" name="PMS_11U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(11,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Integrated Features -> Video (G):</h4> 
			...
			if(isset($_SESSION['p_seVal10'])!=""){
               ...
               if($_SESSION['p_seVal10']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX13=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal10'])!=""){
               ...
               if($_SESSION['p_seVal10']==$MValue_VName){ echo "selected"; }
              }else{
               
               if($PMX13==$MValue_id){ echo "selected"; } 
              }
            ?>><?=$MValue_VName;?></option>
            <?php
            }
...
<h4>Integrated Features -> LAN (N):</h4> 
			...
			if(isset($_SESSION['p_seVal11'])!=""){
               ...
               if($_SESSION['p_seVal11']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX14=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal11'])!=""){
               ...
               if($_SESSION['p_seVal11']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX14==$MValue_id){ echo "selected"; } 
...
<h4>Integrated Features -> RAID (R):</h4> 
			...
			if(isset($_SESSION['p_seVal12'])!=""){
               ...
               if($_SESSION['p_seVal12']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX15=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal12'])!=""){
               ...
               if($_SESSION['p_seVal12']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX15==$MValue_id){ echo "selected"; } 
...
<h4>Server Mgmt.:</h4> 
			...
			if(isset($_SESSION['p_seVal13'])!=""){
               ...
               if($_SESSION['p_seVal13']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX16=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal13'])!=""){
               ...
               if($_SESSION['p_seVal13']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX16==$MValue_id){ echo "selected"; } 
...
<h4>RoHS (Type):</h4> 
			...
			if(isset($_SESSION['p_seVal14'])!=""){
               ...
               if($_SESSION['p_seVal14']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX17=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal14'])!=""){
               ...
               if($_SESSION['p_seVal14']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX17==$MValue_id){ echo "selected"; } 
...
<h4>Memory -> Max.:</h4> 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp; <div id="PMT_Show08" style="display:none"><input type="text" id="PMS_08" name="PMS_08" size="20" value=""><br />Tooltips :<input id="PMS_08U" name="PMS_08U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(8,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>HDD -> Max.:</h4> 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp;<!--<font color=red><?=$PMX11;?></font>--> <div id="PMT_Show10" style="display:none"><input type="text" id="PMS_10" name="PMS_10" size="20" value=""><br />Tooltips :<input id="PMS_10U" name="PMS_10U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(10,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Exp. Slots -> PCIx:</h4> 
            ...
            mysqli_close($link_db);
            ?>
            </select>&nbsp;<!--<font color=red><?=$PMX16;?></font>--> <div id="PMT_Show15" style="display:none"><input type="text" id="PMS_15" name="PMS_15" size="20" value=""><br />Tooltips :<input id="PMS_15U" name="PMS_15U" type="text" size="20" value=""  /><input type="button" value="Add New" onclick="Add_Matrvalue(15,<?=$p_SKU;?>,<?=$EPType_id;?>);"></div>
</div>
...
<h4>Exp. Slots -> PCIe:</h4> 
			...
			if(isset($_SESSION['p_seVal15'])!=""){
               ...
               if($_SESSION['p_seVal15']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX18=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal15'])!=""){
               ...
               if($_SESSION['p_seVal15']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX18==$MValue_id){ echo "selected"; } 
...
<h4>Server Mgmt.:</h4> 
			...
			if(isset($_SESSION['p_seVal16'])!=""){
               ...
               if($_SESSION['p_seVal16']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX19=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal16'])!=""){
               ...
               if($_SESSION['p_seVal16']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX19==$MValue_id){ echo "selected"; } 
...
<h4>RoHS (Type):</h4> 
			...
			if(isset($_SESSION['p_seVal17'])!=""){
               ...
               if($_SESSION['p_seVal17']=="NO"){ echo "selected"; }
               
              }else{
              
               if($PMX20=="NO"){ echo "selected='selected'"; }
            ...
            <?php
              if(isset($_SESSION['p_seVal17'])!=""){
               ...
               if($_SESSION['p_seVal17']==$MValue_VName){ echo "selected"; }
               
              }else{
               
               if($PMX20==$MValue_id){ echo "selected"; } 
...
<!--<p class="w12red">* Go to <a href="matrix.php">"Product Matrix"</a> to setup matrix's header and other details.</p>-->
<p>&nbsp;</p><p>&nbsp;</p><p class="clear"></p>
...
mysqli_query($link_db, 'SET CHARACTER_SET_RESULTS=utf8');
//$select=mysqli_select_db($dataBase, $link_db);
$str_nw="select SKUs_Sid,SKUs_Mname from skus_sublist where SKUs_Mid=".$data_CSKUs[0]." and ProductTypeID=".$EPType_id;
$NW_result=mysqli_query($link_db,$str_nw);
while($ndb=mysqli_fetch_array($NW_result)){
$u=$u+1;
if(isset($ndb['SKUs_Mname'])!=''){
?>
<option value="<?=htmlspecialchars($ndb['SKUs_Mname'], ENT_QUOTES)?>" 
<?php
if(trim($SM3)==trim($ndb['SKUs_Mname'])){
echo "selected";
}else if(trim($SM4)==trim($ndb['SKUs_Mname'])){
echo "selected";
}else if(trim($SM5)==trim($ndb['SKUs_Mname'])){ 
echo "selected";
...
?>                        
</select>&nbsp;&nbsp;&nbsp;&nbsp; <div id="SKUPN_add<?=$data_CSKUs[0]?>" style="display:none">Values: <input type="text" id="SSMN1_<?=$data_CSKUs[0]?>" name="SSMN1_<?=$data_CSKUs[0]?>" size="20" value=""> <input type="hidden" name="SSMN2_<?=$data_CSKUs[0]?>" value="<?=$data_CSKUs[0];?>"><input type="hidden" name="SSMN3_<?=$data_CSKUs[0]?>" value="<?=$EPType_id;?>"><input type="hidden" name="SSMN4" value="<?=$data_CKstr;?>"><input id="SSMNBtn<?=$data_CSKUs[0]?>" name="SSMNBtn<?=$data_CSKUs[0]?>" type="button" value="Done" /><div id="SubSKUs_MGT<?=$data_CSKUs[0];?>"></div></div>
...
<input name="aspecLang[]" type="checkbox" value="EN" <?php if(preg_match("/EN/i",$SM12)!='') {echo "checked";} ?> /> English &nbsp;&nbsp;
<input name="aspecLang[]" type="checkbox" value="CN" <?php if(preg_match("/CN/i",$SM12)!='') {echo "checked";} ?> /> 簡中 &nbsp;&nbsp;
<input name="aspecLang[]" type="checkbox" value="ZH" <?php if(preg_match("/ZH/i",$SM12)!='') {echo "checked";} ?> /> 繁中 &nbsp;&nbsp;
<input name="aspecLang[]" type="checkbox" value="JP" <?php if(preg_match("/JP/i",$SM12)!='') {echo "checked";} ?> /> 日本語 &nbsp;&nbsp;
</p>

<p style="padding-left:145px; font-weight:bolder"><input name="specEOL" type="checkbox" value="1" <?php if(ord($SM8)==true) {echo "checked";} ?> /> EOL</p>
...
<input type="hidden" name="SPECC_Sort_tr" value="<?=$data_spec_str;?>" />
