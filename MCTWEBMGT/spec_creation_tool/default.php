...
if(isset($_REQUEST['wstatus'])!='' && isset($_REQUEST['sku_id'])!=''){
  ...
  if($wstatus=='Y' && $sku_id!=''){
      ...
      if($csku_data[0]!=''){
		...
		$instcsku_cmd=mysqli_query($link_db,$str_instcsku);
		//echo $str_instcsku;
...
<strong>Lauguage: </strong><input id="slang" name="slang" type="checkbox" value="EN" <?php if(strrchr($slang,"EN|")==true) { echo "checked"; } ?> /> English | <input id="slang" name="slang" type="checkbox" value="CN" <?php if(strrchr($slang,"CN|")==true) { echo "checked"; } ?> /> 簡中 <input id="slang" name="slang" type="checkbox" value="ZH" <?php if(strrchr($slang,"ZH|")==true) { echo "checked"; } ?> /> 繁中 <input id="slang" name="slang" type="checkbox" value="JP" <?php if(strrchr($slang,"JP|")==true) { echo "checked"; } ?> /> 日本語&nbsp;&nbsp;<input id="s_eol" name="s_eol" type="checkbox" value="1" <?php if($seol=='1'){ echo "checked"; } ?> /> EOL <input id="scheck_checked" name="scheck_checked" type="button" value="Search" onclick="Lang_check();" /> </div>
</div>
<p class="clear"></p>
<div id="content"><h3 class="left">Product SPEC List:</h3>
<p class="clear"></p>
...
<table class="list_table">
    ...
    <th><a href="add_spec.php" STYLE="text-decoration:none"> Add New </a><div class="button14" style="width:80px;" onClick="location='add_spec.php'">Add New</div></th>
	</tr>
  <?php
  ...
  <tr class="list_table_con" >
	...
	?></td>
		<td><?=$MODELCODE;?></td>
		<td>
		<a href="#" onclick="window.open('../mockups/product_spec_tmp.php?sku_id=<?=base64_url_encode($Product_SKU_Auto_ID);?>&sku_name=<?=base64_url_encode($SKU);?>&sku_mcode=<?=base64_url_encode($MODELCODE);?>'); return false"><?=$SKU;?></a></td>        
		<td>
    <?php
	echo "EN";
    ?></td>
