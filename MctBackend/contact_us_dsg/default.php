...
if(isset($_REQUEST['d_id'])!=""){
  ...
  $str_d="delete FROM contact_us_dsg where ID=".$d1;
...
if(isset($_REQUEST['s_ID'])!=''){
  ...
  $str1 = "select count(*) FROM contact_us_dsg WHERE ID = '".$s_ID."' Order by CREATEDATE";
}elseif (isset($_REQUEST['s_Name'])!='') {
  $s_Name=trim($_REQUEST['s_Name']);
  $str1 = "select count(*) FROM contact_us_dsg WHERE NAME = '".$s_Name."' Order by CREATEDATE";
}elseif (isset($_REQUEST['s_Email'])!='') {
  $s_Email=trim($_REQUEST['s_Email']);
  $str1 = "select count(*) FROM contact_us_dsg WHERE EMAIL = '".$s_Email."' Order by CREATEDATE";
}elseif (isset($_REQUEST['s_cName'])!='') {
  $s_cName=trim($_REQUEST['s_cName']);
  $str1 = "select count(*) FROM contact_us_dsg WHERE COMPANYNAME = '".$s_cName."' Order by CREATEDATE";
}elseif (isset($_REQUEST['s_date1'])!='' && isset($_REQUEST['s_date2'])!='') {
  $s_date1=trim($_REQUEST['s_date1']);
  $s_date2=trim($_REQUEST['s_date2']);
  $str1 = "select count(*) FROM contact_us_dsg WHERE CREATEDATE BETWEEN '".$s_date1." 00:00:00' AND '".$s_date2." 00:00:00' Order by CREATEDATE";
}elseif (isset($_REQUEST['s_reg'])!='') {
  $s_reg=trim($_REQUEST['s_reg']);
  if ($_REQUEST['s_reg']=='all') {
    $str1 = "select count(*) FROM contact_us_dsg WHERE 1 Order by CREATEDATE";
  }else{
    $str1 = "select count(*) FROM contact_us_dsg WHERE REGION = '".$s_reg."' Order by CREATEDATE";
  }
}else{
  $str1 = "select count(*) FROM contact_us_dsg WHERE 1 Order by CREATEDATE";
}
...
<body><a name="top"></a>
  ...
  <div id="menu">
    <ul>
      <li ><a href="default.php">Contact Us DSG List</a>
      </li>
    </ul>
  </div>
  <div class="clear"></div>
  <p class="clear"></p>
  <div id="content"><h2>Contact Us DSG List:</h2>
    <p class="clear"></p>
    ...
    <form id="form2" name="form2" method="post" action="default.php">
      <table class="list_table">
       <tr>
        ...
        <th STYLE="text-decoration:none">Subject</th>
      ...
      if(isset($_REQUEST['s_ID'])<>''){
        $s_ID=trim($_REQUEST['s_ID']);
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE ID = '".$s_ID."' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }elseif (isset($_REQUEST['s_Name'])<>'') {
        $s_Name=trim($_REQUEST['s_Name']);
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE NAME = '".$s_Name."' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }elseif (isset($_REQUEST['s_Email'])<>'') {
        $s_Email=trim($_REQUEST['s_Email']);
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE EMAIL = '".$s_Email."' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }elseif (isset($_REQUEST['s_cName'])<>'') {
        $s_cName=trim($_REQUEST['s_cName']);
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE COMPANYNAME = '".$s_cName."' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }elseif (isset($_REQUEST['s_date1'])<>'' && isset($_REQUEST['s_date2'])<>'') {
        $s_date1=trim($_REQUEST['s_date1']);
        $s_date2=trim($_REQUEST['s_date2']);
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE CREATEDATE BETWEEN '".$s_date1." 00:00:00' AND '".$s_date2." 00:00:00' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }elseif (isset($_REQUEST['s_reg'])<>'') {
        $s_reg=trim($_REQUEST['s_reg']);
        if ($_REQUEST['s_reg']=='all') {
         $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE 1 Order by CREATEDATE DESC limit $start_num,$read_num;";
       }else{
        $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE REGION = '".$s_reg."' Order by CREATEDATE DESC limit $start_num,$read_num;";
      }
    }else{
      $str_contact = "select ID, NAME, COMPANYNAME, EMAIL, REGION, Subject, MESSAGE, CREATEDATE FROM contact_us_dsg WHERE 1 Order by CREATEDATE DESC limit $start_num,$read_num;";
    }
