...
$mm_modellist="";$mm_skulist="";$mb_modellist="";$mb_skulist="";$chs_skulist="";$nh_modellist="";$nh_skulist="";$as_modellist="";$as_skulist="";
	...
	if($cid!=''){
	$str_prod="SELECT `ID`, `MODEL`, `UPDATE_USER`, `UPDATE_DATE`, `STATUS` FROM `sp_pressreview` where `ID`=".$cid;
	$prod_result=mysqli_query($link_db,$str_prod);

	$prod_data=mysqli_fetch_row($prod_result);
	$prod_data[1]=preg_replace('/\s(?=)/', '', trim($prod_data[1]));
	}

if(isset($_REQUEST['kinds'])!=""){
if(trim($_REQUEST['kinds'])=="add_sku"){
...
<a href="elb_preview_mo.php?cid=<?=$prod_data[0];?>#BOTTOM"><img src="down-icon.gif" width="52" height="52" title="bottom" /></a>
</div>
...
<ul class="menu">
	...
	<li>
    	<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Server Motherboards</h3></a>
		<ul class="submenu">
        	<!--<p><input name="" type="checkbox" value="" /> [Model]: (<input name="" type="checkbox" value="" /> SKU,&nbsp;<input name="" type="checkbox" value="" /> SKU,)</p>-->
			<li><?php
				$str_MM_model="SELECT `ModelID`, `ModelCode`, `ProductCateID` FROM `product_models` where `ProductCateID` in (9) GROUP BY `ModelCode` order by `ModelCode`";
				$MM_model_record=mysqli_query($link_db,$str_MM_model);				
				$m=0;
				while($MM_model_data=mysqli_fetch_row($MM_model_record)){
				$m+=1;
				?>
				<input id="mm_model[]" name="mm_model[]" type="checkbox" value="S.<?=$MM_model_data[1];?>" <?php if(strpos($prod_data[1], "S.".$MM_model_data[1].",")!='' || strpos($prod_data[1], "S.".$MM_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MM_model_data[1];?>
				  <?php if($MM_model_data==true){?>			  
				  <?php
				  //$str_MM="SELECT `Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE` FROM `product_skus` WHERE `Web_Disable`=0 and `MODELCODE`='".$MM_model_data[1]."' ORDER BY `MODELCODE`";
				  $str_MM="SELECT `Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE` FROM `product_skus` WHERE `MODELCODE`='".$MM_model_data[1]."' ORDER BY `MODELCODE`";
				  ...
				  //$MM_data_Re=str_replace('[BTO]','',$MM_data[2]);
				  ?>
				  <input class="SeMo_Sub<?=$MM_model_data[0];?>" id="mm_sku[]" name="mm_sku[]" type="checkbox" title="<?=$MM_model_data[0];?>" value="<?=$MM_data[2];?>" <?php if(strpos($prod_data[1], $MM_data[2].",")!="" || strpos($prod_data[1], $MM_data[2].",")===0){ echo "checked"; } //if(preg_match("/\b".$MM_data[2].",\b/i",$prod_data[14]) || preg_match("/\b".$MM_data_Re."[[:upper:]],/i",$prod_data[14])) {echo "checked";} ?> /> <?=$MM_data[2];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$MMLast_str;?>
	...
	<li>
    	<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Server Barebones</h3></a>
    	<ul class="submenu">
        	<li><?php
				$str_MB_model="SELECT `ModelID`, `ModelCode`, `ProductCateID` FROM `product_models` where `ProductCateID` in (9) GROUP BY `ModelCode` order by `ModelCode`";
				$MB_model_record=mysqli_query($link_db,$str_MB_model);				
				while($MB_model_data=mysqli_fetch_row($MB_model_record)){
				?>
				<input id="mb_model[]" name="mb_model[]" type="checkbox" value="B.<?=$MB_model_data[1];?>" <?php if(strpos($prod_data[1], "B.".$MB_model_data[1].",")!="" || strpos($prod_data[1], "B.".$MB_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MB_model_data[1];?>
				<?php if($MB_model_data==true){?>
				  
				  <?php
				  $str_MB="SELECT `Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE` FROM `product_skus` WHERE `MODELCODE`='".$MB_model_data[1]."' ORDER BY `MODELCODE`";
				  $MB_record=mysqli_query($link_db,$str_MB);
				  $MB_Tcount=mysqli_num_rows($MB_record);
				  if($MB_Tcount>0){
				  ?>
				  <input class="SeBa<?=$MB_model_data[0];?>" id="mb_model_all[]" name="mb_model_all[]" type="checkbox" value="All" onclick="checkAll_Ba(<?=$MB_model_data[0];?>)" />
				  <?php
				  echo ":  (";
				  $MBLast_str=")";
				  }else{
				  ...
				  while($MB_data=mysqli_fetch_row($MB_record)){
				  ?>
				  <input class="SeBa_Sub<?=$MB_model_data[0];?>" id="mb_sku[]" name="mb_sku[]" type="checkbox" title="<?=$MB_model_data[0];?>" value="<?=$MB_data[2];?>" <?php if(strpos($prod_data[1], $MB_data[2].",")!="" || strpos($prod_data[1], $MB_data[2].",")===0){ echo "checked"; } //if(preg_match("/\b".$MB_data[2].",/i",$prod_data[14])) {echo "checked";} ?> /> <?=$MB_data[2];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$MBLast_str;?>
    ...
    <li>
    	<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Chassis Kit</h3></a>
    	<ul class="submenu">
        	<li><?php				
				//$str_CHS="SELECT p_r_main_rackchassis.SERVERID,Product_SKUs.Product_SKU_Auto_ID,Product_SKUs.SKU,p_r_rackmount.RACKMOUNTNAME,p_r_main_rackchassis.MB_Form_Factors,c_r_powersupply.POWERSUPPLY,'.'+RIGHT (p_r_main_rackchassis.SMALLIMG, LEN(p_r_main_rackchassis.SMALLIMG)-1) as SMALLIMG2,'<img src=.'+RIGHT (p_r_main_rackchassis.SMALLIMG, LEN(p_r_main_rackchassis.SMALLIMG)-1)+'>' AS SMALLIMG3,Product_SKUs.FormFactor,p_r_main_rackchassis.MODELCODE from p_r_main_rackchassis inner join Product_SKUs ON p_r_main_rackchassis.MODELCODE = Product_SKUs.MODELCODE inner join p_r_rackmount on p_r_main_rackchassis.RACKMOUNTID=p_r_rackmount.RACKMOUNTID inner join c_r_powersupply on p_r_main_rackchassis.POWERSUPPLYID=c_r_powersupply.POWERSUPPLYID where p_r_main_rackchassis.LANG='en-US' and (p_r_main_rackchassis.STATUS = '1' and Product_SKUs.Web_Disable=0) order by Product_SKUs.MODELCODE";
				//$str_CHS="SELECT `SERVERID`, `MODELNAME`, `MODELCODE`, `STATUS` FROM `p_r_main_rackchassis`";
				$str_CHS="SELECT a.Product_SKU_Auto_ID, a.SKU, a.MODELCODE, a.Web_Disable FROM `product_skus` a inner join `p_r_main_rackchassis` b on a.MODELCODE=b.MODELCODE where b.LANG='en-US' order by a.MODELCODE";
				$CHS_record=mysqli_query($link_db,$str_CHS);
				$c=0;
				while($CHS_data=mysqli_fetch_row($CHS_record)){
				$c+=1;
				if($c%4==0){
				$br01="<br />";
				}else{
				...
				?>
				<input id="chs_sku[]" name="chs_sku[]" type="checkbox" value="<?=$CHS_data[1];?>" <?php if(strpos($prod_data[1],$CHS_data[2].",")!='' || strpos($prod_data[1],$CHS_data[2].",")===0){ echo "checked"; } //if(eregi($CHS_data[2],$prod_data[14])!='') {echo "checked";} ?> /><?=$CHS_data[1];?>,&nbsp;<?=$br01;?>
				<?php
				}
				?>
			</li>
	...
	<li>
    	...
    	<ul class="submenu">
			<li><?php
				$str_AS_FM="SELECT `FAMILYID`, `FAMILYTYPE`, `LANG`, `STATUS` FROM `p_a_accessfamily` where `LANG`='en-US' and `STATUS`='1'";
				$AS_FM_record=mysqli_query($link_db,$str_AS_FM);
				while($AS_FM_data=mysqli_fetch_row($AS_FM_record)){
				?>
				<input id="as_model[]" name="as_model[]" type="checkbox" value="A.<?=$AS_FM_data[1];?>" <?php if(strpos($prod_data[1], "A.".$AS_FM_data[1].",")!="" || strpos($prod_data[1], "A.".$AS_FM_data[1].",")===0){ echo "checked"; } ?> /> <?=$AS_FM_data[1];?>
				<?php if($AS_FM_data==true){?>
				  
				  <?php
				  $str_AS="SELECT p_a_main_accessories.ACCESSID, p_a_main_accessories.FAMILYID, p_a_accessfamily.FAMILYTYPE, p_a_main_accessories.MODELNAME, p_a_main_accessories.MODELCODE, p_a_main_accessories.MINISPEC, p_a_main_accessories.SPEC, p_a_main_accessories.SMALLIMG, p_a_main_accessories.IMG, p_a_main_accessories.BIGIMG, p_a_main_accessories.COMPATIBLE FROM p_a_main_accessories INNER JOIN p_a_accessfamily ON p_a_main_accessories.FAMILYID = p_a_accessfamily.FAMILYID WHERE (p_a_main_accessories.FAMILYID= ".$AS_FM_data[0].") AND (p_a_main_accessories.STATUS = '1') ORDER BY p_a_main_accessories.MODELNAME,p_a_main_accessories.LAUNCH_DATE, ACCESSID DESC";
				  $AS_record=mysqli_query($link_db,$str_AS);
				  $AS_Tcount=mysqli_num_rows($AS_record);
				  if($AS_Tcount>0){
				  ?>
				  <input class="AcCs<?=$AS_FM_data[0];?>" id="as_model_all[]" name="as_model_all[]" type="checkbox" value="All" onclick="checkAll_Ac(<?=$AS_FM_data[0];?>)" />
				  <?php
				  echo ":  (";
				  $ASLast_str=")";
				  }else{
				  ...
				  while($AS_data=mysqli_fetch_row($AS_record)){
				  ?>
				  <input class="AcCs_Sub<?=$AS_FM_data[0];?>" id="as_sku[]" name="as_sku[]" type="checkbox" title="<?=$AS_FM_data[0];?>" value="<?=$AS_data[3];?>" <?php if(strpos($prod_data[1],$AS_data[3].",")!='' || strpos($prod_data[1],$AS_data[3].",")===0){ echo "checked"; } ?> /> <?=$AS_data[3];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$ASLast_str;?>
