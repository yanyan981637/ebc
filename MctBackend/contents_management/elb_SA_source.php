...
if($Type=="add_sa"){
  ...
  $tt=$mm_modellist;
  echo "<script language='Javascript'>parent.document.forms['form2'].edit_SAsourceID.value='".$tt."';parent.document.getElementById('edit_SAsource').innerHTML='".$tmp."';";
...
<table class="list_table">
  ...
  <tr>
    ...
    <td ><input id="mm_model[]" name="mm_model[]" type="checkbox" value="<?=$data[0];?>" <?php if(strpos($eID,",".$data[0].",")!='' || strpos($eID,",".$data[0].",")===0){ echo "checked"; } ?>/><?=$data[1]?></td>
    <td ><?=$data[2]?></td>
    <td>
