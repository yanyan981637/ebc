...
$mm_modellist="";$mm_skulist="";$mb_modellist="";$mb_skulist="";$chs_skulist="";$nh_modellist="";$nh_skulist="";$as_modellist="";$as_skulist="";$jbo_modellist="";$jbo_skulist="";$tpm_modellist="";$tpm_skulist="";$op_modellist="";$op_skulist="";
	...
	if($nid!=''){
	$str_prod="SELECT `ActivityID`, `Activity_Subject`, `title`, `Summary`, `IMG`, `Source_URL`, `Relate_Prod`, `crea_d`, `crea_u`, `upd_d`, `upd_u`, `LANG`, `STATUS` FROM `nr_newsletter` where `ActivityID`=".$nid;
...
echo "  {";
echo "    parent.window.opener.document.forms['form2'].relProd_mval.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist.$jbo_skulist.$tpm_modellist.$tpm_skulist.$op_modellist.$op_skulist."'";
echo "  }";
echo "  ";
	...
	<li>
		...
		<ul class="submenu">
			<li><?php
				...
				$m+=1;
				?>
				<input id="mm_model[]" name="mm_model[]" type="checkbox" value="S.<?=$MM_model_data[1];?>" <?php if(strpos($prod_data[6], "S.".$MM_model_data[1].",")!='' || strpos($prod_data[6], "S.".$MM_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MM_model_data[1];?>
				  <?php if($MM_model_data==true){?>
				  ...
				  $MM_data[2]=preg_replace('/\s(?=)/', '', trim($MM_data[2]));
				  ?>
				  <input class="SeMo_Sub<?=$MM_model_data[0];?>" id="mm_sku[]" name="mm_sku[]" type="checkbox" title="<?=$MM_model_data[0];?>" value="<?=$MM_data[2];?>" <?php if(eregi(htmlspecialchars($MM_data[2], ENT_QUOTES),htmlspecialchars($prod_data[6], ENT_QUOTES))!='') {echo "checked";} ?> /> <?=$MM_data[2];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$MMLast_str;?>
