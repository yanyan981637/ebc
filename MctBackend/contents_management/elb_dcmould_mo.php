...
if(isset($_REQUEST['cid'])!='' && isset($_REQUEST['d_type'])!=''){

	$cid01=intval($_REQUEST['cid']);
	$d_type01=$_REQUEST['d_type'];
	if($d_type01=="Manuals"){
	$str_m="SELECT `MODEL` from `sp_manual` where `ID`=".$cid01;
	}else if($d_type01=="Datasheets"){
	$str_m="SELECT `MODEL` from `sp_datasheet` where `ID`=".$cid01;
	}else if(trim($d_type01)=="FRU"){
	$str_m="SELECT `MODEL` from `sp_fru` where `ID`=".$cid01;
	}else if(trim($d_type01)=="VideoCompatibility"){
	$str_m="SELECT `MODEL` from `sp_videocompatibility` where `ID`=".$cid01;
	}else if(trim($d_type01)=="-"){
	$str_m="SELECT `MODEL` from `sp_fru` where `ID`=".$cid01;
	}
...
if(isset($_POST['jbod_model'])!=''){
foreach($_POST['jbod_model'] as $jbod_model01){
 $jbod_modellist=$jbod_modellist.$jbod_model01.",";
}
}else{
 $jbod_modellist="";
}
...
echo "  {";
//echo "    parent.window.opener.document.getElementById('relProd_valM').innerHTML = '".$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist."'";
//echo "    parent.window.opener.document.getElementById('relProd_valM').value = '".$mm_skulist."'";
echo "    parent.window.opener.document.forms['form2'].relProd_valM.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist.$jbod_modellist.$jbo_skulist.$tpm_modellist.$tpm_skulist.$op_modellist.$op_skulist."'";
...
<div id="abgne_float_ad" >
		...
		<a href="elb_dcmould_mo.php?cid=<?=$cid01;?>&d_type=<?=$d_type01;?>#BOTTOM">
