...
if($pid!=''){
	$str_prod="SELECT `MODEL` FROM `nr_pressroom` where `ID`=".$pid;
	$prod_result=mysqli_query($link_db,$str_prod);
	$m_data=mysqli_fetch_row($prod_result);
}
//$prod_data[14]
...
echo "<script language='Javascript'>parent.document.forms['form2'].relProd_mval.value='".$tt."';";
/*echo "<script language='Javascript'>";
echo "try{";
echo "  if(parent.window.opener != null && !parent.window.opener.closed)";
echo "  {";
echo "    parent.window.opener.document.forms['form2'].relProd_mval.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist."'";
echo "  }";
echo "  ";
