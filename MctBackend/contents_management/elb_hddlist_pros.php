...
$mm_modellist="";$mm_skulist="";$mb_modellist="";$mb_skulist="";$chs_skulist="";$nh_modellist="";$nh_skulist="";$as_modellist="";$as_skulist="";$jbo_modellist="";$jbo_skulist="";$tpm_modellist="";$tpm_skulist="";$op_modellist="";$op_skulist="";
	...
	if($cid!=''){
	$str_prod="SELECT `ID`, `VENDER_NAME`, `HDD_SIZE`, `NOTE`, `HDD_TYPE`, `HDD_CAPACITY`, `HDD_BUS`, `LANG`, `MODEL`, `UPDATE_USER`, `UPDATE_DATE`, `STATUS`, `MODEL_NAME` FROM `sp_hdd` where `ID`=".$cid;
	$prod_result=mysqli_query($link_db,$str_prod);

	$prod_data=mysqli_fetch_row($prod_result);
	$prod_data[8]=preg_replace('/\s(?=)/', '', trim($prod_data[8]));
	}
	//echo $prod_data[8];
	...
	<li>
		...
		<ul class="submenu">
			...
			<li><?php
				...
				$m+=1;
				?>
				<input id="mm_model[]" name="mm_model[]" type="checkbox" value="S.<?=$MM_model_data[1];?>" <?php if(strpos($prod_data[8],"S.".$MM_model_data[1].",")!='' || strpos($prod_data[8],"S.".$MM_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MM_model_data[1];?>
				  ...
				  ?>
				  <input class="SeMo_Sub<?=$MM_model_data[0];?>" id="mm_sku[]" name="mm_sku[]" type="checkbox" title="<?=$MM_model_data[0];?>" value="<?=$MM_data[2];?>" <?php if(strpos($prod_data[8], $MM_data[2].",")!="" || strpos($prod_data[8], $MM_data[2].",")===0){ echo "checked"; } //if(preg_match("/\b".$MM_data[2].",\b/i",$prod_data[14]) || preg_match("/\b".$MM_data_Re."[[:upper:]],/i",$prod_data[14])) {echo "checked";} ?> /> <?=$MM_data[2];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$MMLast_str;?>
