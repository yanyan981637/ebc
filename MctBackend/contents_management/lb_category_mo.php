...
if(isset($_GET['ca_id'])!=''){
	$product_type_ID=dowith_sql($_GET['ca_id']);
}elseif(isset($_GET['kinds'])!=''){

}else{
	echo "No Product Type ID";exit();
}

if(isset($_GET['kinds'])!=''){
	if($_GET['kinds']=="add_sku"){
		...
		if(isset($_POST['op_model'])!=''){
			foreach($_POST['op_model'] as $op_model01){
				$op_modellist=$op_modellist.$op_model01.",";
			}
		}else{
			$op_modellist="";
		}
		...
		echo "  {";
		//echo "    parent.window.opener.document.getElementById('relProd').innerHTML = '".$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist."'";
		//echo "    parent.window.opener.document.getElementById('relProd_val').value = '".$mm_skulist."'";
		echo "    parent.window.opener.document.forms['form1'].relProd_val.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist.$jbo_modellist.$jbo_skulist.$tpm_modellist.$tpm_skulist.$op_modellist.$op_skulist."'";
...
<body style="backbround:#f9f9f9">
	...
	<!-- <div id="abgne_float_ad" >
			...
			<a href="lb_doload_mo.php#BOTTOM">
				<img src="down-icon.gif" width="52" height="52" title="bottom" />
			</a>
	</div> -->

	<div>
		<?php
		$str_count="SELECT count(DISTINCT(a.Product_SContents_Auto_ID)) FROM `contents_product_skus` a inner join `product_models` b on a.MODELCODE=b.ModelCode where a.ProductTypeID='".$product_type_ID."'";
		$count_result = $pdo->prepare($str_count);
		$count_result->execute();
		$cdata=$count_result->fetch(PDO::FETCH_NUM);
		?>
		<div class="pagination left">Total: <span class="w14bblue"><?=$cdata[0];?></span> records &nbsp;&nbsp;</div> 
	</div>
	<p class="clear">&nbsp;</p>
		<form id="form_support" name="form_support" method="post" action="?kinds=add_sku">
			<div id="accordion">
				<ul class="menu">
					<?php
					if($product_type_ID==46){  //producttypes_las
					?>
					<li>
		    		<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Industrial Panel PC</h3></a>
						<ul class="submenu">
							<li><?php
								...
								$cmd = $pdo->prepare($str_MM_model);
								$cmd->execute();
								$m=0;
								while($MM_model_data=$cmd->fetch(PDO::FETCH_NUM)){
								$m+=1;
								?>
								<input id="mm_model[]" name="mm_model[]" type="checkbox" value="S.<?=$MM_model_data[1];?>" <?php if(strpos($m_data[0],"S.".$MM_model_data[1].",")!='' || strpos($m_data[0],"S.".$MM_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MM_model_data[1];?>
								  ...
								  $str_MM="SELECT `Product_SKU_Auto_ID`, `ProductTypeID`, `SKU`, `MODELCODE` FROM `product_skus` WHERE `MODELCODE`='".$MM_model_data[1]."' ORDER BY `MODELCODE`";
								 	$MM_record = $pdo->prepare($str_MM);
									$MM_record->execute();
								  $MM_Tcount=$MM_record->rowCount();
								  if($MM_Tcount>0){
								  ?>
								  ...
								  while($MM_data=$MM_record->fetch(PDO::FETCH_NUM)){
					...
					if($product_type_ID==47){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==48){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==49){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==50){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==51){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==52){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==53){  //producttypes_las
					?>
					<li>
			    	<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;POS</h3></a>
						<ul class="submenu">
					...
					if($product_type_ID==54){  //producttypes_las
					?>
					<li>
					...
					if($product_type_ID==55){  //producttypes_las
					?>
					<li>
			    	<a href="#"><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Intel DSG</h3></a>
						<ul class="submenu">
				        	<!--<p><input name="" type="checkbox" value="" /> [Model]: (<input name="" type="checkbox" value="" /> SKU,&nbsp;<input name="" type="checkbox" value="" /> SKU,)</p>-->
							<li><?php
								$str_MM_model="SELECT `ModelID`, `ModelCode`, `ProductCateID` FROM `product_models` where `ProductCateID` in (10) GROUP BY `ModelCode` order by `ModelCode`"; //cateID table : productcategories
								$cmd = $pdo->prepare($str_MM_model);
								$cmd->execute();
								$m=0;
								while($MM_model_data=$cmd->fetch(PDO::FETCH_NUM)){
			...
			<p class="clear">&nbsp;</p><p><input name="R1" type="reset" value="Cancel"  />&nbsp;&nbsp;&nbsp;&nbsp;<input name="B1" type="submit" value="Done"  /></p>
		</form>
		<div id="BOTTOM"></div>
		<P style="color:#0F0">
