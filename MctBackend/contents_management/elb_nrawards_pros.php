...
$mm_modellist="";$mm_skulist="";$mb_modellist="";$mb_skulist="";$chs_skulist="";$nh_modellist="";$nh_skulist="";$as_modellist="";$as_skulist="";
	...
	if($aid!=''){
	$str_award="select `ID`, `AWARDDATE`, `MEDIA`, `COUNTRY`, `CONTENT`, `LINK`, `MODEL`, `IMAGES`, `DETAIL`, `LANG`, `STATUS` from `nr_award` where `ID`=".$aid;
	$award_result=mysqli_query($link_db,$str_award);

	$award_data=mysqli_fetch_row($award_result);
	$award_data[6]=preg_replace('/\s(?=)/', '', trim($award_data[6]));
	}
	//echo $award_data[6];
...
echo "  {";
echo "    parent.window.opener.document.forms['form2'].relProd_val.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist."'";
echo "  }";
echo "  ";
