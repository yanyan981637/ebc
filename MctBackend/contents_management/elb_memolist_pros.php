...
$mm_modellist="";$mm_skulist="";$mb_modellist="";$mb_skulist="";$chs_skulist="";$nh_modellist="";$nh_skulist="";$as_modellist="";$as_skulist="";$jbo_modellist="";$jbo_skulist="";$tpm_modellist="";$tpm_skulist="";$op_modellist="";$op_skulist="";
	...
	if($cid!=''){
	$str_prod="SELECT `ID`, `VENDER_NAME`, `MEMORY_SIZE`, `NOTE`, `MEMORY_TYPE`, `CHIP`, `VENDER_NUMBER`, `CHIP_PART_NUMBER`, `MEMORY_FREQUENCE`, `NAME`, `AMB`, `VOLTAGE`, `ROHS`, `LANG`, `MODEL`, `QualifiedCPU`, `UPDATE_USER`, `UPDATE_DATE`, `STATUS` FROM `sp_memory` where `ID`=".$cid;
	$prod_result=mysqli_query($link_db,$str_prod);

	$prod_data=mysqli_fetch_row($prod_result);
	$prod_data[14]=preg_replace('/\s(?=)/', '', trim($prod_data[14]));
	}
	//echo $prod_data[14];
...
echo "  {";
//echo "    parent.window.opener.document.getElementById('relProd').innerHTML = '".$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist."'";
//echo "    parent.window.opener.document.getElementById('relProd_val').value = '".$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist."'";
echo "    parent.window.opener.document.forms['form1'].relProd_val.value = '".$mm_modellist.$mm_skulist.$mb_modellist.$mb_skulist.$chs_skulist.$nh_modellist.$nh_skulist.$as_modellist.$as_skulist.$jbo_modellist.$jbo_skulist.$tpm_modellist.$tpm_skulist.$op_modellist.$op_skulist."'";
//echo "$('#relProd_val', parent.opener.window.document).attr('value','".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist."')";
echo "  }";
echo "  ";
	...
	<li>
		...
		<ul class="submenu">
			...
			<li><?php
				...
				$m+=1;
				?>
				<input id="mm_model[]" name="mm_model[]" type="checkbox" value="S.<?=$MM_model_data[1];?>" <?php if(strpos($prod_data[14],"S.".$MM_model_data[1].",")!='' || strpos($prod_data[14],"S.".$MM_model_data[1].",")===0){ echo "checked"; } ?> /> <?=$MM_model_data[1];?>
				  ...
				  ?>
				  <input class="SeMo_Sub<?=$MM_model_data[0];?>" id="mm_sku[]" name="mm_sku[]" type="checkbox" title="<?=$MM_model_data[0];?>" value="<?=$MM_data[2];?>" <?php if(strpos($prod_data[14], $MM_data[2].",")!="" || strpos($prod_data[14], $MM_data[2].",")===0){ echo "checked"; } //if(preg_match("/\b".$MM_data[2].",\b/i",$prod_data[14]) || preg_match("/\b".$MM_data_Re."[[:upper:]],/i",$prod_data[14])) {echo "checked";} ?> /> <?=$MM_data[2];?>,&nbsp;
				  <?php
				  }
				  ?>
				  <?=$MMLast_str;?>
