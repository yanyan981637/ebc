...
if(isset($_REQUEST['cid'])!='' && isset($_REQUEST['d_type'])!=''){
	...
	$cid01=(int)$_REQUEST['cid'];
	$d_type01=trim($_REQUEST['d_type']);	
	
	if($d_type01=="Drivers"){
	$str_m="SELECT `MODEL` from `sp_driver` where `ID`=".$cid01;
	}else if($d_type01=="BIOS"){
	$str_m="SELECT `MODEL` from `sp_bios` where `ID`=".$cid01;
	}else if(trim($d_type01)=="Firmware"){
	$str_m="SELECT `MODEL` from `sp_firmware` where `ID`=".$cid01;
	}else if(trim($d_type01)=="Press Review"){
	$str_m="SELECT `MODEL` from `sp_pressreview` where `ID`=".$cid01;
	}else if(trim($d_type01)=="Software Downloads"){
	$str_m="SELECT `MODEL` from `sp_software` where `ID`=".$cid01;
	}
...
echo "    parent.window.opener.document.forms['form2'].relProd_valM.value = '".$mm_modellist.$mb_modellist.$nh_modellist.$as_modellist.$mm_skulist.$mb_skulist.$chs_skulist.$nh_skulist.$as_skulist.$jbo_modellist.$jbo_skulist.$tpm_modellist.$tpm_skulist.$op_modellist.$op_skulist."'";
