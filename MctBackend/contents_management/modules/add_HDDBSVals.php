...
$str_New="select `ID` from `c_sp_hdd_interface` order by `ID` desc limit 1";
...
if($MV_str01==''){
   ...
   $str_chk="SELECT `ID` FROM `c_sp_hdd_interface` where `interface`='".$MV_str01."'";
   $chk_cmd=mysqli_query($link_db,$str_chk);
   $chk_record=mysqli_fetch_row($chk_cmd);
   
   if(empty($chk_record)):        

	  $str_val="insert into `c_sp_hdd_interface` (`ID`, `interface`, `DESCRIPTION`, `STATUS`) values (".$MCount.",'".$MV_str01."','".$MV_str01."','1')";
