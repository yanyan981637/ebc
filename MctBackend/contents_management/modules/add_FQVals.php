...
$str_New="select `ID` from `c_sp_memory_frequence` order by `ID` desc limit 1";
...
if($MV_str01=='' || $MV_str02==''){
   ...
   $str_chk="SELECT `ID` FROM `c_sp_memory_frequence` where `FREQUENCE`='".$MV_str01."' and `DESCRIPTION`='".$MV_str02."' and `STATUS`=1";
   $chk_cmd=mysqli_query($link_db,$str_chk);
   $chk_record=mysqli_fetch_row($chk_cmd);
   
   if(empty($chk_record)):        

	  $str_val="insert into `c_sp_memory_frequence` (`ID`, `FREQUENCE`, `DESCRIPTION`, `STATUS`, `Order_val`) values (".$MCount.",'".$MV_str01."','".$MV_str02."','1','".$add_order."')";
