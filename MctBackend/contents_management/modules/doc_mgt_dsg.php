...
if(isset($_REQUEST['kinds'])!=''){
  if($_REQUEST['kinds']=='del'){
	  $d_id01=intval($_REQUEST['d_id']);
	  $str_del="delete from `sp_doc_dsg` where `ID`=".$d_id01;
	  $del_cmd=mysqli_query($link_db,$str_del);
	  echo "<script>alert('Delete The Data!');self.location='doc_mgt_dsg.php'</script>";
	  exit();
...
if(isset($_REQUEST['kinds'])!=""){
	if(trim($_REQUEST['kinds'])=="edit_docmt"){
		...
		if(isset($_POST['dl_title'])!=""){
			$dl_title=trim($_POST['dl_title']);
		}else{
			$dl_title="";
		}
		if(isset($_POST['edit_brandtype'])!=""){
			$edit_brandtype=trim($_POST['edit_brandtype']);
		}else{
			$edit_brandtype="";
		}
		...
		/*if($dl_type01==6){
			...
			$doc_type="TPS"; //Technical Product Specification
		}elseif($dl_type01==22){
			$doc_type="ISG"; //Integration and Service Guide
		}elseif($dl_type01==23){
			$doc_type="DOC"; //Declaration of Conformity
		}elseif($dl_type01==24){
			$doc_type="MDDS"; //Material Declaration Data Sheet 	
		}elseif($dl_type01==25){
			$doc_type="Others"; 
		}*/
		...
		$str_upd="UPDATE `sp_doc_dsg` SET `FILENAME`='".$dl_name01."',`FILEDATE`='".$dl_Date01."',`DESCRIPTION`='".$dl_desc01."',`VERSION`='".$dl_ver01."',`PATH`='".$dl_loc01."',`FILESIZE`='".$dl_size01."',`NOTES`='".$dl_note01."',`LANG`='en-US',`MODEL`='".$relProd_val."',`UPDATE_USER`='webmaster',`UPDATE_DATE`='".$now."',`STATUS`='".$status01."',`Doc_Type`='".$dl_type01."',`Title`='".$dl_title."',`brand_type`='".$edit_brandtype."' WHERE `ID`=".$m_id01;
		$upd_cmd=mysqli_query($link_db,$str_upd);
		echo "<script>alert('Update The Data!');location.href='doc_mgt_dsg.php'</script>";
		exit();
	}
	...
	if(trim($_REQUEST['kinds'])=="add_docmt"){
		...
		if(isset($_POST['add_brandtype'])!=''){
			$add_brandtype=trim($_POST['add_brandtype']);
		}else{
			$add_brandtype="";
		}
		...
		if(isset($_POST['dlA_title'])!=''){
			$dlA_title=trim($_POST['dlA_title']);
		}else{
			$dlA_title="";
		}
		...
		/*if($dlA_type01==6){
			...
			$doc_type="Datasheets";
		}elseif($dlA_type01==21){
			$doc_type="TPS"; //Technical Product Specification
		}elseif($dlA_type01==22){
			$doc_type="ISG"; //Integration and Service Guide
		}elseif($dlA_type01==23){
			$doc_type="DOC"; //Declaration of Conformity
		}elseif($dlA_type01==24){
			$doc_type="MDDS"; //Material Declaration Data Sheet 	
		}elseif($dlA_type01==25){
			$doc_type="Others"; 
		}*/

		/*$str_MULvalues="select `ID` FROM `sp_doc_dsg` order by `ID` desc limit 1";
		$check_MULvalues=mysqli_query($link_db,$str_MULvalues);
		$Max_CValID=mysqli_fetch_row($check_MULvalues);
		$MCount=$Max_CValID[0]+1;*/

		if($dlA_note01!=''){
			$str_inst="INSERT INTO `sp_doc_dsg`(`FILENAME`, `FILEDATE`, `DESCRIPTION`, `VERSION`, `PATH`, `FILESIZE`, `NOTES`, `LANG`, `MODEL`, `STATUS`, `Doc_Type`, `Title`, `brand_type`) VALUES ('".$dlA_name01."','".$dlA_Date01."','".$dlA_desc01."','".$dlA_ver01."','".$dlA_loc01."','".$dlA_size01."','".$dlA_note01."','en-US','".$relProd_val."','".$statusA01."','".$dlA_type01."','".$dlA_title."','".$add_brandtype."')";
		}else{
			$str_inst="INSERT INTO `sp_doc_dsg`(`FILENAME`, `FILEDATE`, `DESCRIPTION`, `VERSION`, `PATH`, `FILESIZE`, `LANG`, `MODEL`, `STATUS`, `Doc_Type`, `Title`, `brand_type`) VALUES ('".$dlA_name01."','".$dlA_Date01."','".$dlA_desc01."','".$dlA_ver01."','".$dlA_loc01."','".$dlA_size01."','en-US','".$relProd_val."','".$statusA01."','".$dlA_type01."','".$dlA_title."','".$add_brandtype."')";
		}
		$inst_cmd=mysqli_query($link_db,$str_inst);
		echo "<script>alert('AddNew The Data!');location.href='doc_mgt_dsg.php'</script>";
		exit();
	}
}
...
if(isset($_REQUEST['s_search'])<>''){
  ...
  //$s_search=preg_replace("['\"\$ \r\n\t;<>\*%\?]", '', $_REQUEST['s_search']);
	if($_REQUEST['Sel_DType']<>''){
		...
		if($sel_dtype==6 || $sel_dtype==7 || $sel_dtype==21 || $sel_dtype==22 ||$sel_dtype==23 || $sel_dtype==24 || $sel_dtype==25){
			if($sel_dtype==6){
				$doc_type="Manuals";
			}elseif($sel_dtype==7){
				$doc_type="Datasheets";
			...
			$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE Doc_Type='".$doc_type."' AND f.FILENAME like '%".$s_search."%' or f.DESCRIPTION like '%".$s_search."%'";
		}else{
			$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE f.FILENAME like '%".$s_search."%' or f.DESCRIPTION like '%".$s_search."%' or f.Title like '%".$s_search."%' or f.Title like '%".$s_search."%'";
		}
	}else{
		$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE f.FILENAME like '%".$s_search."%' or f.DESCRIPTION like '%".$s_search."%' or f.Title like '%".$s_search."%'";
	}
}else{
	...
	if(isset($_REQUEST['Sel_DType'])<>''){
		...
		if($sel_dtype==6 || $sel_dtype==7 || $sel_dtype==21 || $sel_dtype==22 ||$sel_dtype==23 || $sel_dtype==24 || $sel_dtype==25){
			...
			$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE Doc_Type='".$doc_type."'";
		}else{
			$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE 1";
		}
	}else{	 
		$str1="SELECT count(f.FILENAME) FROM sp_doc_dsg f WHERE 1";
	}  
...
function MM_PT(selObj){
	...
	if(dl_num==6  || dl_num==7 || dl_num==21 || dl_num==22 || dl_num==23 || dl_num==24 || dl_num==25){
		$("#tr_1").show();$("#tr_1_01").hide();$("#tr_1_02").hide();$("#tr_1_03").hide();$("#tr_1_04").show();$("#tr_1_05").show();$("#tr_1_06").show();$("#tr_1_07").show();$("#tr_1_08").show();$("#tr_1_09").show();$("#tr_1_10").show();
		$("#tr_2").hide();
		$("#tr_3").hide();
		$("#tr_4").hide();
		document.getElementById('download_pannel').style.display='block';
		document.getElementById('cha_vmodel').style.display='none';
		document.getElementById('dlA_title').style.display='block';
...
function MM_PTM(selObj){
  ...
  if(dl_num== 6 || dl_num==7 || dl_num==21 || dl_num==22 || dl_num==23 || dl_num==24 || dl_num==25){
  	$("#tr_1M").show();$("#tr_1_01M").hide();$("#tr_1_02M").hide();$("#tr_1_03M").hide();$("#tr_1_04M").show();$("#tr_1_05M").show();$("#tr_1_06M").show();$("#tr_1_07M").show();$("#tr_1_08M").show();$("#tr_1_09M").show();$("#tr_10").show();
  	$("#tr_2M").hide();
  	$("#tr_3M").hide();
  	$("#tr_4M").hide();
  	document.getElementById('download_pannelM').style.display='block';
  	document.getElementById('cha_vmodelM').style.display='none';
  	document.getElementById('dl_title').style.display='block';
...
<?php
$str_dl="SELECT `ID`, `NAME` FROM `c_sp_itemlist` where `ID` in (6,7,21,22,23,24,25)";
...
<table class="list_table">
  ...
  if(isset($_REQUEST['s_search'])<>''){
  	...
  	if($_REQUEST['Sel_DType']<>''){
  		...
  		if($sel_dtype==6 || $sel_dtype==7 || $sel_dtype==21 || $sel_dtype==22 ||$sel_dtype==23 || $sel_dtype==24 || $sel_dtype==25){
  			...
  			$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE f.Doc_Type='".$doc_type."' AND f.FILENAME like '%".$s_search."%' or f.DESCRIPTION like '%".$s_search."%' or f.Title like '%".$s_search."%' ORDER BY f.FILEDATE desc limit $start_num,$read_num;";
  		}else{
  			$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE f.FILENAME like '%".$s_search."%' or f.Title like '%".$s_search."%' ORDER BY f.FILEDATE DESC limit $start_num,$read_num;";
  		}
  	}else{
  		$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE f.FILENAME like '%".$s_search."%' or f.Title like '%".$s_search."%' ORDER BY f.FILEDATE DESC limit $start_num,$read_num;";
  	}
  	...
  	if(isset($_REQUEST['Sel_DType'])<>''){		  
  		...
  		if($sel_dtype==6 || $sel_dtype==7 || $sel_dtype==21 || $sel_dtype==22 ||$sel_dtype==23 || $sel_dtype==24 || $sel_dtype==25){
  			...
  			$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE f.Doc_Type='".$doc_type."' ORDER BY f.FILEDATE DESC limit $start_num,$read_num;";
  		}else{
  			$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE 1 ORDER BY f.FILEDATE DESC limit $start_num,$read_num;";
  		}
  	}else{		
  		$str="SELECT f.ID, f.FILENAME, f.Doc_Type, f.FILEDATE, f.MODEL, f.STATUS FROM sp_doc_dsg f WHERE 1 ORDER BY f.FILEDATE DESC limit $start_num,$read_num;";
  	}
  ...
  $result=mysqli_query($link_db,$str);
	$i=0;
  while(list($ID,$FILENAME,$Doc_Type,$FILEDATE,$MODEL,$STATUS)=mysqli_fetch_row($result)){
  $i=$i+1;
  putenv("TZ=Asia/Taipei");
  if($Doc_Type==6){
  	$d_type=6; 
  	...
  	$d_type=21; 
  	$Doc_Type="Technical Product Specification";
  }elseif($Doc_Type==22){ 
  	$d_type=22; 
  	$Doc_Type="Integration and Service Guide";
  }elseif($Doc_Type==23){ 
  	$d_type=23;  
  	$Doc_Type="Declaration of Conformity";
  }elseif($Doc_Type==24){
  	$d_type=24; 
  	$Doc_Type="Material Declaration Data Sheet";
  }elseif($Doc_Type==25){
  	$d_type=25; 
  	$Doc_Type="Others";
  }
  ...
  <tr>
	...
	<td><?=$MODEL;?></td><td><?php if($STATUS==1){ echo "Online"; }else if($STATUS==0){ echo "Offline"; } ?></td>
	<td ><a id="dw_edit" href="?mid=<?=$ID;?>&d_type=<?=$d_type;?>#docmt_edit">Edit</a>&nbsp;&nbsp;<a href="?kinds=del&d_id=<?=$ID;?>" onclick="return confirm('Are you sure you want to delete this item?');">Del</a></td>
  </tr> 
  <?php
	  }
  ?>
...
<?php
$str_dltpA="SELECT `ID`, `NAME` FROM `c_sp_itemlist` where `ID` in (6,7,21,22,23,24,25)";
...
<tr id="tr_1_10">
<th>Title:  </th>
<td><input id="dlA_title" name="dlA_title" type="text" size="40" value="" style="display:none" /></td>
</tr>
<tr id="tr_1_04">
<th>File Name:  </th>
...
<th>Status:</th>
<td>
	<select id="statusA" name="statusA"><option value="1" selected>Online</option><option value="0">Offline</option></select>
	&nbsp;&nbsp;&nbsp;
...
<tr><td colspan="2">
<input name="B2" class="button14" style="width:60px;" type="submit" value="Done" />&nbsp;&nbsp;<input name="C2" class="button14" style="width:60px;" type="button" value="Cancel" onclick="javascript:location.href='doc_mgt_dsg.php'" /> <span style="color:#0F0">Cancel: 不save, 直接close 這個add/edit box</span>
</td></tr>
</table>
</form>
<script language="JavaScript">
...
$mid=$_GET['mid'];
$d_type01=$_GET['d_type'];
$str_m="SELECT `ID`, `FILENAME`, `FILEDATE`, `DESCRIPTION`, `VERSION`, `PATH`, `FILESIZE`, `NOTES`, `LANG`, `MODEL`, `STATUS`, `Doc_Type`, `Title`, `brand_type` FROM `sp_doc_dsg` WHERE `ID`='".$mid."' AND Doc_Type='".$d_type01."'";
...
$Doc_Type=$m_data[11];
$title=$m_data[12];
$brand_type=$m_data[13];

if($Doc_Type=="TPS"){
	$d_type01="Technical Product Specification"; //Technical Product Specification
}elseif($Doc_Type=="SISG"){
	$d_type01="System Integration and Service Guide"; //System Integration and Service Guide
}
?>
...
<?php
//$str_dltp="SELECT `ID`, `NAME` FROM `c_sp_itemlist` where `ID` in (6,7,20,21,22,23,24,25) and `NAME`='".trim($d_type01)."'";
$str_dltp="SELECT `ID`, `NAME` FROM `c_sp_itemlist` where `ID`='".$Doc_Type."'";
...
<tr id="tr_1_10M">
<th>Title:  </th>
<td><input id="dl_title" name="dl_title" type="text" size="50" value="<?=$title;?>" style="display:none" /></td>
</tr>
<tr id="tr_1_04M">
<th>File Name:  </th>
...
<tr>
<th>Products:</th>
<td><div class="button14" style="width:60px;" ><a class="fancybox fancybox.iframe" href="../elb_doload_mo_dsg.php?cid=<?=$id01?>&d_type=doc" style="color:#ffffff">Edit</a></div>
 <!--列出被勾選的Products-->
...
<td><select id="status" name="status"><option value="1" <?php if($status01=='1'){ echo "selected"; } ?>>Online</option><option value="0" <?php if($status01=='0'){ echo "selected"; } ?>>Offline</option></select>
	...
	<select id="edit_brandtype" name="edit_brandtype">
		<option value="Intel" <?php if($brand_type=='Intel'){ echo "selected"; } ?>>Intel</option>
		<option  value="MCT" <?php if($brand_type=='MCT'){ echo "selected"; } ?>>MCT</option>
	</select>
</td>
</tr>
<tr><td colspan="2">
