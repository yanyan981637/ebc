...
if($MVm_str01==''){
   ...
   /*
   $str_chk="SELECT `ID` FROM `c_sp_hdd_interface` where `interface`='".$MVm_str01."'";
   $chk_cmd=mysqli_query($link_db,$str_chk);
   $chk_record=mysqli_fetch_row($chk_cmd);
   
   if(empty($chk_record)):        
   */
	  $str_val="update `c_sp_hdd_interface` set `interface`='".$MVm_str01."',`UPDATE_USER`='webmaster',`UPDATE_DATE`='$now',`STATUS`='1' where `ID`=".$MValue_id;
