...
if(isset($_REQUEST['act'])!=''){
  ...
  if($d_id01!=''){
  $str_del="delete from `spdescription_list` where `ID`=".$d_id01;
  $del_cmd=mysqli_query($link_db,$str_del);
  echo "<script>alert('Delete The Data!');self.location='pro_descrp.php'</script>";
  exit();
  }
  }
}
...
if(trim($_REQUEST['kinds'])=="edit_pdesc"){
...
if(isset($_POST['smryM'])!=""){
$smryM=trim($_POST['smryM']);
}else{
$smryM="";
}
...
$str_upd="UPDATE `spdescription_list` SET `NAME`='".$name01."',`DESCS`='".$descM."',`smmarys`='".$smryM."',`LANG`='".$langM."',`MODEL`='".$relProd_val."',`UPDATE_USER`='webmaster',`UPDATE_DATE`='".$now."',`STATUS`='".$status01."' WHERE `ID`=".$m_id01;
$upd_cmd=mysqli_query($link_db,$str_upd);
echo "<script>alert('Update The Data!');location.href='pro_descrp.php'</script>";
exit();
}


if(trim($_REQUEST['kinds'])=="add_pdesc"){
...
if(isset($_POST['smryA'])!=''){
$smryA01=trim(str_replace("'","&#39;",$_POST['smryA']));
}else{
$smryA01="";
}
...
$str_MULvalues="select `ID` FROM `spdescription_list` order by `ID` desc limit 1";
$check_MULvalues=mysqli_query($link_db,$str_MULvalues);
$Max_CValID=mysqli_fetch_row($check_MULvalues);
$MCount=$Max_CValID[0]+1;
	
if($descA!=''){
$str_inst="INSERT INTO `spdescription_list`(`ID`, `NAME`, `DESCS`, `smmarys`, `LANG`, `MODEL`, `STATUS`, `UPDATE_USER`, `UPDATE_DATE`) VALUES (".$MCount.",'".$name01A."','".$descA."','".$smryA01."','".$langA."','".$relProd_val."','".$statusA01."','webmaster','".$now."')";
}else{
$str_inst="INSERT INTO `spdescription_list`(`ID`, `NAME`, `smmarys`, `LANG`, `MODEL`, `STATUS`, `UPDATE_USER`, `UPDATE_DATE`) VALUES (".$MCount.",'".$name01A."','".$smryA01."','".$langA."','".$relProd_val."','".$statusA01."','webmaster','".$now."')";
}
$inst_cmd=mysqli_query($link_db,$str_inst);
echo "<script>alert('AddNew The Data!');location.href='pro_descrp.php'</script>";
exit();
}
}
$slang="";
if(isset($_REQUEST['s_search'])<>'' && trim($_REQUEST['s_search'])!=""){
  ...
  //$s_search=preg_replace("['\"\$ \r\n\t;<>\*%\?]", '', $_REQUEST['s_search']);
	...
	$str1="select count(ID) from `spdescription_list` where (NAME like '%".$s_search."%' or `DESCS` like '%".$s_search."%') and `LANG`='".$slang."'";
	}else{
	$str1="select count(ID) from `spdescription_list` where NAME like '%".$s_search."%' or `DESCS` like '%".$s_search."%'";
	}
  }else{
	if(isset($_REQUEST['slang'])!='' && trim($_REQUEST['slang'])!=""){		  
    $slang=trim($_REQUEST['slang']);
	$str1="select count(ID) from `spdescription_list` where `LANG`='".$slang."'";
	}else{
	$str1="select count(ID) from `spdescription_list`";
	}
  }
...
<p>
<form id="form3" name="form3" method="post" action="pro_descrp.php" onsubmit="return false;">
<select id="SEL_SLang" name="SEL_SLang" onChange="MM_SL(this)">
<option value="pro_descrp.php?slang=">All</option>
<option value="pro_descrp.php?slang=en-US" <?php if($slang=='en-US'){ echo "selected";} ?>>English</option>
<option value="pro_descrp.php?slang=zh-CN" <?php if($slang=='zh-CN'){ echo "selected";} ?>>簡體</option>
<option value="pro_descrp.php?slang=zh-TW" <?php if($slang=='zh-TW'){ echo "selected";} ?>>繁體</option>
<option value="pro_descrp.php?slang=ja-JP" <?php if($slang=='ja-JP'){ echo "selected";} ?>>日文</option>
...
<table class="list_table">
  ...
  <?php
	  ...
	  //$s_search=preg_replace("['\"\$ \r\n\t;<>\*%\?]", '', $_REQUEST['s_search']);
	    ...
	    if(isset($_REQUEST['slang'])!='' && trim($_REQUEST['slang'])!=""){
		$slang=trim($_REQUEST['slang']);
		$str="SELECT `ID`, `NAME`, `MODEL`, `LANG`, `STATUS` FROM `spdescription_list` where (NAME like '%".$s_search."%' or `DESCS` like '%".$s_search."%') and `LANG`='".$slang."' ORDER BY `UPDATE_DATE` desc limit $start_num,$read_num;";
		}else{
		$str="SELECT `ID`, `NAME`, `MODEL`, `LANG`, `STATUS` FROM `spdescription_list` where NAME like '%".$s_search."%' or `DESCS` like '%".$s_search."%' ORDER BY `UPDATE_DATE` desc limit $start_num,$read_num;";
	    }
	  }else{
		if(isset($_REQUEST['slang'])!='' && trim($_REQUEST['slang'])!=""){
		$slang=trim($_REQUEST['slang']);
		$str="SELECT `ID`, `NAME`, `MODEL`, `LANG`, `STATUS` FROM `spdescription_list` where `LANG`='".$slang."' ORDER BY `UPDATE_DATE` desc limit $start_num,$read_num;";
		}else{
	    $str="SELECT `ID`, `NAME`, `MODEL`, `LANG`, `STATUS` FROM `spdescription_list` ORDER BY `UPDATE_DATE` desc limit $start_num,$read_num;";
	    }
	  }
  ...
  ?>
  <tr>
    <td><?php echo $NAME; ?> </td><td><?=$MODEL;?></td><td><?=$LANG;?></td><td><?php if($STATUS==1){ echo "Online"; }else if($STATUS==0){ echo "Offline"; } ?></td><td ><a id="dw_edit" href="?mid=<?=$ID;?> #pdesc_edit">Edit</a>&nbsp;&nbsp;<a href="?act=del&d_id=<?=$ID;?>" onclick="return confirm('Are you sure you want to delete this item?');">Del</a></td>
  </tr> 
  <?php
	  }
  ?>
...
<form id="form1" name="form1" method="post" action="?kinds=add_pdesc" onsubmit="return Final_Check();">							
<h1>Add a Product Description</h1>
...
<th>Summary:</th>
<td>
<textarea id="smryA" name="smryA" rows="4" cols="50" style="max-width: 250px; max-height: 250px;"></textarea>
</td>
</tr>
<tr>
<th>Status:</th>
...
if(isset($_REQUEST['mid'])!=''){
  ...
  $str_m="SELECT `ID`, `NAME`, `DESCS`, `smmarys`, `LANG`, `MODEL`, `STATUS` FROM `spdescription_list` where `ID`=".$mid;
...
<form id="form2" name="form2" method="post" action="?kinds=edit_pdesc" enctype="multipart/form-data" onsubmit="return Final_MCheck();">
<input type="hidden" name="m_id" value="<?=$id01;?>">
<h1>Edit a Product Description</h1>
...
<tr>
<th>Products:</th>
<td><div class="button14" style="width:60px;" ><a class="fancybox fancybox.iframe" href="../elb_prodesc_mo.php?cid=<?=$id01;?>" style="color:#ffffff">Edit</a></div>
 <!--列出被勾選的Products-->
...
<th>Summary:</th>
<td>
<textarea id="smryM" name="smryM" rows="4" cols="50" style="max-width: 250px; max-height: 250px;"><?=$smmarys01;?></textarea>
</td>
</tr>
<tr>
<th>Status:</th>
...
<tr><td colspan="2">
<input name="B2" class="button14" style="width:60px;" type="submit" value="Done" />&nbsp;&nbsp;<input name="C2" class="button14" style="width:60px;" type="button" value="Cancel" onclick="javascript:location.href='pro_descrp.php'" />
</td></tr>
</table>
</form>
