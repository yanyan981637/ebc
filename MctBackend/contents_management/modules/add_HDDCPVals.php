...
$str_New="select `ID` from `c_sp_hdd_capacity` order by `ID` desc limit 1";
...
if($MV_str01==''){
   ...
   $str_chk="SELECT `ID`, `Capacity`, `STATUS` FROM `c_sp_hdd_capacity` where `Capacity`='".$MV_str01."'";
   $chk_cmd=mysqli_query($link_db,$str_chk);
   $chk_data=mysqli_fetch_row($chk_cmd);
   
   if(empty($chk_data)):
     
	  $str_val="insert into `c_sp_hdd_capacity` (`ID`, `Capacity`) values (".$MCount.",'".$MV_str01."')";
