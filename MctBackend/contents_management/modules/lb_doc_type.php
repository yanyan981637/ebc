...
if(trim($_REQUEST['act'])=='del'){
  if($_REQUEST['d_id']!=''){
	...
	$str_del="delete from `c_sp_itemlist` where `ID`=".$did01;
	$del_cmd=mysqli_query($link_db,$str_del);
	
	$str_dclist="delete FROM `document_list` where `DTYPE_ID`=".$did01;
	$dclist_cmd=mysqli_query($link_db,$str_dclist);
...
$str1="select count(*) from `c_sp_itemlist` where `TYPE_VAL`='DC'";
...
<p class="clear"></p>
<form id="form2" name="form2" method="post" enctype="multipart/form-data" action="lb_doc_type.php">
...
<!--add a HDD/SSD Interface-->
  <tr>
    <td colspan="2">
	<DIV id="MValue_ADD01" style="display:none"><input id="MV_str01" name="MV_str01" type="text" size="18" value="" /> <input type="file" id="MyFile" name="MyFile" size="30" /> <input id="MVaBtn" type="button" value="Done" /><input id="MVaCel" type="button" value="Cancel" /><DIV id="MVal_MGT"></DIV><DIV id="MVal_error"></DIV></div>
	<?php
	if(isset($_REQUEST['mid'])!=''){
	 $mid01=intval($_REQUEST['mid']);
	 $str_MVm="SELECT `ID`, `NAME`, `LINK`, `IMG`, `TYPE_VAL` FROM `c_sp_itemlist` where `ID`=".$mid01;
	 $MVm_cmd=mysqli_query($link_db,$str_MVm);
	 $MVm_data=mysqli_fetch_row($MVm_cmd);
	?>
	<DIV id="MValue_MOD01" style="display:none"><input id="MValue_id" name="MValue_id" type="hidden" value="<?=$_REQUEST['mid'];?>"><input id="MVm_str01" name="MVm_str01" type="text" size="18" value="<?=$MVm_data[1];?>" /> <input type="file" id="MyFilem" name="MyFilem" size="30" /> <input id="MVaBtnm" type="button" value="Done" /><input id="MVaCelm" type="button" value="Cancel" /><DIV id="MValm_MGT"></DIV><DIV id="MValm_error"></DIV></div>
	<?php
	}
	?>
	</td>	
...
<!--end add a HDD/SSD Interface-->
  ...
  $str_m1="SELECT `ID`, `NAME`, `LINK`, `IMG`, `TYPE_VAL` FROM `c_sp_itemlist` where `TYPE_VAL`='DC' order by `NAME` limit $start_num,$read_num;";
