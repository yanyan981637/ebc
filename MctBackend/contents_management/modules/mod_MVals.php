...
if($MVm_str01==''){
   echo "<font color=red><b>Data be empty!</b></font>";
	  ...
	  if($MyFilem!=''){
	  $str_val="UPDATE `c_sp_memory_modulevender` SET `MODULEVENDER`='".$MVm_str01."',`ICON`='$url$MyFilem',`URL`='".$MVm_str02."',`UPDATE_USER`='admin',`UPDATE_DATE`='".$now."',`STATUS`=1 where `ID`=".$MValue_id;
	  }else{
	  $str_val="UPDATE `c_sp_memory_modulevender` SET `MODULEVENDER`='".$MVm_str01."',`URL`='".$MVm_str02."',`UPDATE_USER`='admin',`UPDATE_DATE`='".$now."',`STATUS`=1 where `ID`=".$MValue_id;
	  }
