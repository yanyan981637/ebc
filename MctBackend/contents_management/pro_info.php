...
if($act=='flg_set'){
 ...
 if($Pvla_if_id!=''){
	...
	$str_Uset="update `product_infovalue_las` set `PIV_flag`=".$flags." where `PIV_id`=".$Pvla_if_id;
...
if($act=='del'){
$pro_if_id=intval($_REQUEST['pro_if_id']);
$str_del="delete from `product_info_las` where `product_info_las`=".$pro_if_id;
...
$str_insert="insert into `product_info_las` (`PI_id`, `PI_Name`, `slang`, `PI_Value`, `PTYPE_Value`, `Sorts`) values (".$MCount.",'".$INA01."','".$IN_LANG."','".$PV_lists01."','".$ppts01."',".$stat01.")"; 
...
$str_update="update `product_info_las` set `PI_Name`='".$INA01m."', `slang`='".$IN_LANGm_val."', `PTYPE_Value`='".$pptm01."', `Sorts`=".$stat01m." where `PI_id`=".$PV_str00m;
//echo $str_update;
...
$pro_if_id01=intval($_REQUEST['pro_if_id']);
$str_m="SELECT `PI_id`, `PI_Name`, `slang`, `PI_Value`, `PTYPE_Value`, `Sorts` FROM `product_info_las` where `PI_id`=".$pro_if_id01;
...
<!--end add a Value-->
  <?php
   ...
   $str_Vlistm="SELECT `PIV_id`, `PIV_Value`, `PIV_Sort`, `PIV_flag` FROM `product_infovalue_las` where `PIV_id`=".$PI_Vlistm_ar[$s]." and `PI_id`=".$mdata[0]." order by `PIV_Sort` desc";
  ...
  ?>
  <tr>
    <td ><?=$Vlistm_vals?>&nbsp;&nbsp;<a href="<?=$Valuem_flg?>"><?=$Vlistm_Data[2];?></a></td><td ><?=$Vlistm_Data[1];?></td><td width="250">&nbsp;&nbsp;<a href="?Del_Pvalm=Del&D_id=<?=$Vlistm_Data[0];?>&pro_if_id=<?=$mdata[0];?>">Del</a>&nbsp;&nbsp;<a href="?Pvla_if_id=<?=$Vlistm_Data[0];?>&pro_if_id=<?=$mdata[0];?>#pro_info_mod">Edit</a><?=$str_Vlistm;?></td>
  </tr>
  <?php
   }
   }
