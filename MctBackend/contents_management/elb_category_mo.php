...
if(isset($_GET['ca_id'])!='' && isset($_GET['pt_mid'])!=''){
	$cid01=(int)$_GET['ca_id'];
	$product_type_ID=(int)$_GET['pt_mid'];
}

if(isset($_REQUEST['kinds'])=="mod_sku"){
...
<title>Edit Supported Products</title>
...
<body style="backbround:#f9f9f9">
	...
	<div>
		...
		$str_m="SELECT Models FROM category_module_las where CategoryModuID=".$cid01;
		$mcmd = $pdo->prepare($str_m);
		$mcmd->execute();
		$m_data=$mcmd->fetch(PDO::FETCH_NUM);
		?>
	...
	<p class="clear">&nbsp;</p>
	<form id="form_support" name="form_support" method="post" action="?kinds=mod_sku">
		<div id="accordion">
			<ul class="menu">
				<?php
