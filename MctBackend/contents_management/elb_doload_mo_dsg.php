...
if(isset($_REQUEST['cid'])!='' && isset($_REQUEST['d_type'])!=''){
	...
	if($d_type01=="download"){
	$str_m="SELECT `Models` from `dsg_download` where `ID`=".$cid01;
	}else if($d_type01=="doc"){
	$str_m="SELECT `MODEL` from `sp_doc_dsg` where `ID`=".$cid01;
	}else if($d_type01=="art"){
	$str_m="SELECT `Products` from `article` where `ID`=".$cid01;
	}
