...
if($kind=="register"){
  ...
  $Verification=date("Y/m/d H:i:s", strtotime("+1 months", strtotime($now)));
    ...
    $str_inst_u.=" VALUES ('".$CompanyID."','".$username."','".$companyname."','".$email."','".$password."','".$countryCode."','".$tel."','".$Msg."','0','1','".$now."')";
  ...
  $mail->SMTPAutoTLS = false;
    ...
    $admail->SMTPAutoTLS = false;
      ...
      $srt_mapping="SELECT ID, Sales, CountryCode FROM partner_mapping WHERE (CountryCode LIKE '%".$countryCode.",%' OR CountryCode LIKE '%,".$countryCode.",%' OR CountryCode LIKE '%,".$countryCode."%') OR CountryCode = '".$countryCode."'";
      $cmd_mapping=mysqli_query($link_db,$srt_mapping);
