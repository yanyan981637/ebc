...
if($kind=="Login"){
  if($_POST['mail']!=""){
    $acc_mail=trim(dowith_sql($_POST['mail']));
    $acc_mail=filter_var($acc_mail);
  }else{
    $acc_mail="";
  }
  if($_POST['topPW']!=""){
    $password=trim(dowith_sql($_POST['topPW']));
  ...
  if($_POST['keepIn']!=""){  // keep login
    $keepIn=trim(dowith_sql($_POST['keepIn']));
    $keepIn=filter_var($keepIn);
  }else{
    $keepIn="";
  }

  if($_POST['RFQpage']!=""){
    $RFQpage=trim(dowith_sql($_POST['RFQpage']));
    $RFQpage=filter_var($RFQpage);
  }else{
    $RFQpage="";
  }
  ...
  if(isset($_SESSION["Checknum"])!=''){
    if($_SESSION["Checknum"]==$Checknum1){
      ...
      if($keepIn=="1"){ // keep login
        $key = 'T!Y^A@N*';
        $encrypt = encrypt($user[2], $key);
        //setcookie("cuser",$encrypt,time()+3600*24*7, "/");
        //setcookie("IN", "1", time()+3600*24*7, "/");// login status
        $_SESSION['IN']="1";
      }else{
        $datetime="";
        //setcookie("cuser","",time()-3600*24*7);
        //setcookie("IN", "1", time()+3600*24*1, "/");// login status
        $_SESSION['IN']="1";
      }

      //setcookie("RFQsku","",time()-3600*24*7);

      if($user[4]=="1" && $RFQpage!="Y"){
              ...
              <table style='width: 570px; margin: 0 auto; padding: 0; -premailer-width: 570px; -premailer-cellpadding: 0; -premailer-cellspacing: 0; background-color: #FFFFFF;' align='center' width='570' cellpadding='0' cellspacing='0'>
                ...
                <tr>
                  <td style='padding: 35px;'>
                    ...
                    <p style='font-family: arial; line-height:130%;  text-align:left; font-size:14px'>Your password for logging in Partner Zone has been reset to:<br /><span style='font-weight:bold; color:#000000; font-size:14px'>".$password."</span></p>
                    <!-- Action -->
                    <table style='width: 100%;  margin: 30px auto;  padding: 0;  -premailer-width: 100%;  -premailer-cellpadding: 0;  -premailer-cellspacing: 0;  text-align: center;' align='center' width='100%' cellpadding='0' cellspacing='0'>
                      <tr>
                        <td align='center'>
                          ...
                          <p style='font-family: arial; line-height:130%;  text-align:left; font-size:14px'>Please use this new password to log in Partner Zone. For your account security, please change your password after login. </p>
                          <br />
...
if($kind=="logout"){
  ...
  //清除Session
  session_destroy();
  //setcookie("IN", "", time()-3600, '/', "mitacmct.com");// login status
  echo "success";
  mysqli_close($link_db);
  exit();
}

/*function addRFQ($RFQ_SKU, $Qnum, $UID,$link_db,$select)
{
  putenv("TZ=Asia/Taipei");
  $now=date("Y/m/d H:i:s");
  if($RFQ_SKU!=""){
    ...
    foreach ($arr_sku as $key => $value) {
      if($value!=""){
        ...
        $str1="SELECT Product_SKU_Auto_ID, SKU, MODELCODE, Quote, ProductTypeID FROM product_skus WHERE SKU='".$value."' AND Product_SKU_Auto_ID>'700000000'";
        $cmd1=mysqli_query($link_db,$str1);
        $result=mysqli_fetch_row($cmd1);
        $arrPID.=$result[0].",";
        $arrPTID.=$result[4].",";
        $arrSKU.=$result[1].",";
        $arrMODEL.=$result[2].",";
    ...
    $str_UID="SELECT ID, CompanyName, Email, CompanyID, Name, ResponsibleSales, CountryCode FROM partner_user WHERE ID='".$UID."'";
    ...
    $user_content = "
                          ...
                          <tr>
                            <td align='center'>
                              ...
                              <p style='font-family: arial; line-height:130%;  text-align:left; font-size:14px'>
                               ...
                               Our sales representative ".$Susername."(".$Semail.") will contact with you shortly via telephone or email to check your requirements before becoming a Tyan partner.
                               <br />
      ...
      $mail->Subject = "Thank you for registering MiTAC Partner Zone"; //設定郵件標題
  ...
  $mail->SMTPAutoTLS = false;
      ...
      if(!$mail->Send()) {
        ...
        $admail->Subject = "New lead notification(TO user)"; //設定郵件標題
        $admail->Body = $errorMail; //設定郵件內容
        $admail->IsHTML(true); //設定郵件內容為HTML
    $admail->SMTPAutoTLS = false;
        $admail->AddAddress("nick.t@tyan.com.tw", "Nick.t"); //設定收件者郵件及名稱
        //$admail->AddCC("even.syao@tyan.com.tw", "even.syao");
        $admail->Send();
        $errMsg="Mailer Error(Mapped): " . $mail->ErrorInfo;
        mysqli_close($link_db);
        return $errMsg;
      }else{
        $ture="susses";
      ...
      There is a new request from ".$companyname."(".$CompanyID.").<br />
      ...
      You can <a href='https://www.tyan.com/partner-backend/' />log into MiTAC Partner Zone Back-end</a> and go to <strong>'Leads Mgt'</strong> to check / proceed this request.
  ...
  $mail->SMTPAutoTLS = false;
    ...
    $admail->SMTPAutoTLS = false;
        ...
        //$admail->AddCC("even.syao@tyan.com.tw", "even.syao");
        $admail->Send();
        $errMsg="Mailer Error(Company): " . $mail->ErrorInfo;
        mysqli_close($link_db);
        return $errMsg;
      }else{
        $ture="susses";
