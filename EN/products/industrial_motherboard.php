		...
		<div class="container-fluid clearfix">
			...
			<div class="row gutter-60 mt-2">
				...
				<ul id="left-m" class="list-group list-group-flush">
					<li class="list-group-item <?=$status?>" onclick="location.href='/EN/products/industrial_motherboard/'">ALL</li>
					<?php
					$strFF="SELECT PIV_id, PI_id, PIV_Value, PIV_Sort FROM product_infovalue_las WHERE PI_id='1'";
					$cmdFF=mysqli_query($link_db,$strFF);
					while ($dataFF=mysqli_fetch_array($cmdFF)){
						...
						echo "<li class='list-group-item ".$status."' onclick=location.href='/EN/products/industrial_motherboard@".$dataFF[0]."=Search'>".$dataFF[2]."</li>";
					}
					?>
				</ul>
			...
			<div class="col-lg-10">
				...
				<div id="portfolio" class="portfolio row gutter-20 col-mb-30 grid grid-container customjs" data-layout="fitRows">
					...
					$IS_BTO="";$PR_info="";
					if($info==""){
						$strPR="SELECT a.Product_SContents_Auto_ID,a.ProductTypeID,a.MODELCODE,a.STATUS,a.Product_Info,a.SKU,a.ProductFile,a.ProductSFile,a.IsnewUp,b.IS_EOL,b.IS_BTO,a.Coming_Soon,a.LandingTitle, a.FormFactor_val, b.ProductTypeID, b.COMPARE, b.REQUEST_QUOTE FROM contents_product_skus a inner join product_skus b on a.Product_SContents_Auto_ID=b.Product_SKU_Auto_ID";
				...
				if($nums!=""){
					...
					$url="/IndustrialMotherboard_".$dataPR[2]."_".$SKU;
					$IsnewUp=$dataPR[8];
					$IS_EOL=$dataPR[9];
					if($dataPR[10]==1){
						$IS_BTO="(BTO)";
						...
						<article class="portfolio-item col-12 col-sm-6 col-lg-4" >
							...
							<div class="grid-inner card card-hover border-0 shadow-sm p-5">
								<div class="portfolio-image">
									<a href="<?=$url?>" /><img src="/images/product/IndustriaMB/<?=$img[0]?>" alt="<?=$dataPR[5]?> Industrial Motherboard "></a>
								</div>
								<div class="portfolio-desc">
									<h3><a href="<?=$url?>"><?=$dataPR[5]?></a>&nbsp;&nbsp;<?=$IS_BTO?></h3>
									<span><?=$LandingTitle?></span>
									<strong><?=$Info[$tmpFF[0]]?></strong>
									<br />
									<div class="line" style="margin: 1rem 0;"></div>
									<div style="font-size:0.9rem">
										<?php
										$i=0;
										$strInfoTitle="SELECT PI_id, PI_Name FROM product_info_las WHERE PI_id IN ('3','4','5','6','7','8','9','10')";//Form Factor ID 1
										$strInfoTitle.=" ORDER BY INSTR(',3,4,5,6,7,8,9,10,',CONCAT(',',PI_id,','))";
										$cmdInfoTitle=mysqli_query($link_db, $strInfoTitle);
										while ($dataInfoTitle=mysqli_fetch_array($cmdInfoTitle)) {

											if($dataInfoTitle[0]!=9 && $dataInfoTitle[0]!=10){
